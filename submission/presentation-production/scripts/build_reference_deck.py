...
# 07 – Radial feature composition around a raised memory core.
s=base('',3,0,[7])
# Circuit-like connectors go behind the four feature compartments.
for x,y in [(3.66,2.79),(9.66,2.79),(3.66,5.35),(9.66,5.35)]:
    line(s,6.67,4.15,x,y,'268AAE',1.5)
clone(s,1,5,5.03,4.10,3.15,1.85);chip(s,5.67,3.38,1.80,'PIXIU')
for x,y,n,t,b in [(.62,2.05,'01','持续积累','授权目录与多源内容\n有出处的知识整理'),(8.32,2.05,'02','有据可查','按问题找回相关信息\n从回答核对原始来源'),(.62,4.62,'03','偏好适配','当前有效偏好的使用\n变化历史与版本来路'),(8.32,4.62,'04','可信协作','授权记忆的跨设备共享\n断连期间的本地工作')]:
    frame(s,x,y,4.35,1.65);medallion(s,n,x+.19,y+.30,.78)
    arttext(s,t,x+1.15,y+.19,2.96,.43,25,PP_ALIGN.LEFT)
    txt(s,b,x+1.15,y+.87,2.97,.69,16,WHITE)
foot(s,'资料积累、来源核对、偏好适配与设备协作，形成连续使用体验')

# 08 – Directory: huge screenshot plus vertical flow.
s=base('照常保存文件，后台接续整理',3,0,[8])
panel(s,.65,1.95,8.0,3.37);shot(s,'directory-folder.png',.85,2.17,7.6,2.89)
for i,(t,b) in enumerate([('先授权','选择目录，保存采集设置'),('再读取','文件稳定后，按内容块处理'),('后复用','知识与来源进入个人记忆')]):
    y=2.0+i*1.48;label(s,f'{i+1:02}  {t}',9.05,y,3.62);txt(s,b,9.1,y+.65,3.55,.58,18)
center(s,'文档 → 分块理解 → 关联已有知识 → 报告处理进度',.75,5.67,7.85,.62,21,C)
foot(s,'用户维护熟悉的文件，PIXIU 接续知识整理')
note(s,'采集需授权；关闭采集不会自动删除已有记忆。')
...
for x,t,items in [(.65,'0.1.12 · V11 已有实证',['系统双 SDK 增删查','目录整理与更正审批','安装、签名升级与故障恢复','47 条恢复一致 / 三端断连并发']), (6.95,'专项待测范围',['最终同版四项量化指标','更多真实业务与对照实验','常驻资源、CPU 与索引增长','物理设备及复杂网络环境'])]:
...
arttext(s,'持续记忆 · 可信协作',.6,2.85,12.1,.9,44)
center(s,'让每一台设备的记忆，彼此相通。',.6,4.13,12.1,.65,26,WHITE,False)
for i,t in enumerate(['持续积累','有据可查','可信协作']):label(s,t,2.16+i*3.05,5.65,2.8)
