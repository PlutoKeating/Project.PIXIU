...
TITLES = ['PIXIU · 貔貅','汇报导览','跨设备记忆场景','需求缺口与方案回应','桌面智能体记忆中枢','业务与技术全景','两大核心亮点','记忆焕新','场景示例 · 账单检索','偏好演进与版本历史','记忆互联','持续记忆技术体系','多源知识接入','知识结构与来源关联','三路混合检索','记忆阶段流转','双层冲突治理','去中心化同步协议','全生命周期安全','智能体记忆生命周期','原生部署与兼容路径','真实操作案例','活动资料与任务要素','记忆焕新与跨会话召回','记忆焕新与更正审批','场景应用价值','开发基线量化评测','记忆互联与三端收敛','验证结果与待测范围','服务模式与持续运营','商业试点路线','PIXIU · 貔貅']
SIDE = {5,9}
...
arttext(s,'记忆焕新 · 记忆互联',1,2.52,11.3,.65,34)
...
for i,(t,b) in enumerate([('场景需求','持续记忆的使用背景'),('方案架构','智能体与记忆的分工'),('功能亮点','记忆焕新与记忆互联'),('技术实现','数据、算法与边界'),('案例验证','完整操作与量化证据'),('商业探索','部署服务与试点路径')]):
    y=2.45+i*.62
...
a=arttext(s,'记忆焕新',7.12,5.06,4.8,.76,43,PP_ALIGN.LEFT)
for r in a.text_frame.paragraphs[0].runs:r.font.italic=True;r._r.get_or_add_rPr().set('spc','160')
a=arttext(s,'记忆互联',8.22,5.98,4.7,.76,43,PP_ALIGN.LEFT)
for r in a.text_frame.paragraphs[0].runs:r.font.italic=True;r._r.get_or_add_rPr().set('spc','160')
...
for i,(t,b) in enumerate([('书房 · 保存账单','收到清单，交代一次\n希望以后还能找回'),('客厅 · 核对开销','只记得“水电燃气”\n需要金额，也需要依据'),('随身 · 更正遗忘','修正错误，停止使用旧内容\n换设备也能接着处理')]):
    x=.62+i*4.2;clone(s,1,5,x,3.8,3.8,1.8);device(s,x+.47,2.55,2.85,t.split(' · ')[0]);center(s,t,x,5.25,3.8,.45,24,C);center(s,b,x,5.86,3.8,.6,16,WHITE,False)
note(s,'林先生家庭账单为需求故事；图示为设备与使用场景说明。')
...
for i,(t,b) in enumerate([('记忆焕新','后台理解资料，关联既有记忆\n新建知识，提出更正与合并'),('记忆互联','可信设备之间共享 Agent 记忆\n各端独立保存，重连补齐差异')]):
    y=2.14+i*2.10
    arttext(s,t,8.98,y,3.78,.56,29,PP_ALIGN.LEFT)
    txt(s,b,9.01,y+.80,3.80,1.12,17)
center(s,'来源追溯 · 偏好适配 · 授权控制',8.85,6.26,4.05,.40,16,C)
note(s,'记忆焕新对应受控 Dreaming：私人范围内后台理解与整合，更正和合并须审批。记忆互联对应 shared 范围的分布式 Agent 记忆共享；私人记忆不自动共享。')
...
for x,n,t,d in [(.60,'01','记忆焕新','后台理解与记忆整合'),(6.94,'02','记忆互联','分布式多设备 Agent 记忆共享')]:
    medallion(s,n,x+.24,2.27,.70)
    arttext(s,t,x+1.12,2.27,4.26,.60,35,PP_ALIGN.LEFT)
...
note(s,'产品名称：记忆焕新。技术名称：Dreaming。目录监视是当前触发入口，核心是模型驱动的受控记忆整合。后台仅处理任务授权的私人资料；不宣称睡眠调度、无依据联想或模型参数训练。更正与合并必须读取原记忆、冻结版本并经过用户审批。截图为更正审批实拍，不作为合并实测证明。')
...
for x,t,items in [(.65,'0.1.12 · V11 已有实证',['系统双 SDK 增删查','记忆焕新与更正审批','安装、签名升级与故障恢复','47 条恢复一致 / 三端断连并发']), (6.95,'专项待测范围',['最终同版四项量化指标','更多真实业务与对照实验','常驻资源、CPU 与索引增长','物理设备及复杂网络环境'])]:
...
arttext(s,'记忆焕新 · 记忆互联',.6,2.85,12.1,.9,44)
center(s,'让每一台设备的记忆，彼此相通。',.6,4.13,12.1,.65,26,WHITE,False)
for x,t,d in [(1.15,'记忆焕新','后台理解与记忆整合'),(7.05,'记忆互联','分布式多设备 Agent 记忆共享')]:
...
NAV_GROUPS = [
    ('场景需求', [('使用场景',[3]),('需求回应',[4])]),
    ('方案架构', [('产品定位',[5]),('技术全景',[6])]),
    ('功能亮点', [('两大亮点',[7]),('记忆焕新',[8]),('场景示例',[9]),('偏好演进',[10]),('记忆互联',[11])]),
