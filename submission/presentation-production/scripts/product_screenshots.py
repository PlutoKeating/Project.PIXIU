...
def window(c,key,x,y,w,h,caption):
    ...
    c.text(caption,x,y+h+.10,w,.52,12,'C4DDED',center=True)
...
def compose(page):
    ...
    if page in {11,30}:
        ...
        for i,(key,domain) in enumerate([('a','kylin-v11'),('b','pixiu-video-b'),('c','pixiu-release-c')]):
            ...
            window(c,key,x,3.87,3.6,2.025,'图 '+str(i+1)+'：设备 '+key.upper()+' 查询同一条共享记忆')
        c.text('三端一致：11 月 10 日下午三点 · 三楼档案室',.9,6.48,11.5,.34,20,GOLD,True,True)
    elif page==10:
        for i,(key,title,cap,body) in enumerate([
            ('devices','记忆共享，分布互连','用户配置可信设备与共享范围','授权共享 · 本地副本 · 重连补齐'),
            ('dreaming','自动记忆，持续整合','用户核对并审批后台生成的更正方案','后台理解 · 关联记忆 · 更正审批')]):
        ...
        window(c,'recall',7.08,2.4,5.60,3.9,'图 1：助手查询已保存的星河活动安排')
    elif page==23:
        ...
        window(c,'task',.9,3.2,7.6,3.12,'图 1：助手完成工具任务并保存处理结果')
        c.notes(NOTES[page],x=8.85,y=3.25,w=3.48,gap=1.45)
    else:
        configs={
          7:('bill','保存资料、持续整合、跨设备共享，让 Agent 的每次任务都能接续已有记忆。','助手在会话中检索并核对账单'),
          12:('dreaming','授权资料变化后，后台理解内容并关联旧记忆；更正与合并交由用户审批。','后台整合资料并生成更正方案'),
          13:('bill','家庭开销核对：清单已交给助手，换个会话询问明细、合计与来源。','助手在新会话中召回账单明细与来源'),
          15:('directory','用户通过会话、附件或授权目录交付资料；工具结果与授权记录进入统一记忆管线。','用户选择授权目录并配置采集范围'),
          16:('source','知识保留结论及其原文、图片或文档块；点击来源即可回看原始依据。','用户查看知识所关联的原始资料'),
          17:('keyword','只记得关键词或大意时，三路召回找回知识，经过 RRF 融合、重排和范围过滤，附上来源。','用户通过关键词检索知识并查看来源'),
          18:('stage','当前任务形成短期记录，阶段内容可继续保留，并按需要转入长期知识。','用户查看短期记录与阶段记忆'),
          19:('dreaming','同一知识的多端修改，与不同记录之间的内容矛盾，需要分别处理。','用户对照新旧内容并审批更正'),
          20:('devices','共享记忆向在线节点传播；离线节点重连时发现缺失，再补齐操作。','用户管理可信设备与同步设置'),
          21:('settings','由用户决定助手可用哪些记忆、哪些内容可以共享；采集和使用均受授权约束。','用户设置助手记忆权限与共享范围'),
          22:('service','麒麟 V11 优先使用系统能力；Debian 提供基本记忆读写与检索的兼容路径。','系统展示当前版本与原生能力状态'),
          25:('source','示例资料：用户将星河观测活动安排放入授权目录，助手读取并保存来源。','助手保留活动知识及其原始资料'),
          26:('recall','资料保存后，在新会话直接提问；助手召回时间、地点和预算，并提供来源链接。','助手在新会话中召回已保存的活动资料'),
          27:('dreaming','活动通知发生变化：用户核对新旧内容，批准后，后续查询使用更新结果。','用户核对活动安排的变化并批准更正'),
          28:('updated','更正后的安排进入后续查询，未变化的预算与准备流程继续保留。','助手在后续查询中使用更正后的安排'),
        }
