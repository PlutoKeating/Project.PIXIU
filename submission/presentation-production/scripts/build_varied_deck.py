...
m['layout_revision']={'source':str(SOURCE.relative_to(WORK.parents[1])),'source_sha256':digest(SOURCE),'parts':parts,'modified_pages':sorted(layouts.PAGES),'preserved_user_pages':[1,34]}
for i,e in enumerate(m['slides'],1):
 if i in result:e['product_evidence']=result[i]
inputs=[SOURCE,WORK/'source/user-cover-closing-20260915-manifest.json',WORK/'reference/ABC公司产品宣传路演PPT.pptx',Path(__file__),WORK/'scripts/varied_layouts.py',WORK/'scripts/product_screenshots.py']
