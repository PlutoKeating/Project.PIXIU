...
export const InsightDocumentScene: React.FC = () => {
  ...
  const focus = interpolate(f, [448, 478], [0, 1], {...clamp, easing: Easing.bezier(0.4, 0, 0.6, 1)});
  const scale = 1.2;
  ...
  return <AbsoluteFill>
    ...
    {f < SHOT_START ? <>
      <div style={{position: 'absolute', left: 135, top: 280, width: 1650, height: 525,
        ...
        <div style={{position: 'absolute', left: 30, top: 245, opacity: f >= 186 ? 1 : 0}}>
          <div style={{fontSize: 36, color: BLUE, marginBottom: 20}}>所选日期的采集日志汇总</div>
          <Crop src="20260909-洞察与当日简报-4K.png" x={760} y={1454} w={1250} h={88} scale={1.25} />
        </div>
      </div>
      <div style={{position: 'absolute', left: 135, top: 830, color: '#526477', fontSize: 36}}>
        {f < 186 ? '本机个人域 · 最近 24 小时 · 可继续检索的候选' : '手动写入产生候选；采集简报只统计采集日志。'}
      </div>
    </> : f < 480 ? <div style={{position: 'absolute', left: 135, top: 280, width: 1650, height: 590, overflow: 'hidden', borderRadius: 16}}>
      <Sequence from={SHOT_START} layout="none">
        <OffthreadVideo src={staticFile('recordings/洞察到来源-4K-02.mp4')} startFrom={90} playbackRate={1.5} muted
      ...
      <div style={{fontSize: 36, lineHeight: 1.8, color: '#526477', marginTop: 70}}>
        候选、检索结果与来源属于同一条演示记忆。<br />标题检索可能命中多条结果，仍需逐条核对来源。
      </div>
    </div>}
    <div style={{position: 'absolute', left: 135, top: 910, fontSize: 36, color: '#526477'}}>
      {f < SHOT_START ? '真实截图逐行揭示 · 非模型生成文档' : f < 480 ? '真实操作录像 · 1.5 倍速 · 裁切放大跟随' : '真实来源正文放大 · 公开合成示例'}
    </div>
  </AbsoluteFill>;
};
