...
export const SharingBoundaryScene: React.FC = () => {
  ...
  const privateStage = frame < 206;
  const sensitiveStage = frame >= 206 && frame < 299;
  const pauseStage = frame >= 299 && frame < 419;
  const confirmStage = frame >= 419 && frame < 509;
  const pausedFrame = frame - 299;
  const pan = interpolate(pausedFrame, [0, 55, 72, 96, 110], [720, 720, 1750, 1750, 720],
  ...
  return <div style={{position: 'absolute', left: 135, right: 135, top: 165}}>
    ...
    </> : confirmStage ? <div style={{position: 'relative', width: 1299, height: 600, overflow: 'hidden', margin: '35px auto 0'}}>
    ...
    <div style={{position: 'absolute', top: 745, left: 0, fontSize: 36, color: '#526477', transform: 'translateZ(0)'}}>
      {frame < 299 ? '同宿主 V11 虚拟机 · 接口观测摘要，非产品界面' : '真实界面与操作节选 · 状态保存不等于传输抑制性能验收'}
    </div>
  </div>;
};
