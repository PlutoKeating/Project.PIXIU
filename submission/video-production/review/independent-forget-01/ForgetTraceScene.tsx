...
export const ForgetTraceScene: React.FC = () => {
  ...
  const rows = final ? trace.queries.map((q) => ({device: q.device,
    status: '检索为空', detail: '回答为空 · 无来源证据 · 无命中知识'})) : [
  ...
  return <div style={{position: 'absolute', left: 135, right: 135, top: 170}}>
    <div style={{fontSize: 38, color: '#1456b8', fontWeight: 700, marginBottom: 26}}>
      {final ? '重连后的核对：三端查询均不再命中' : rejoined ? '恢复连接，客厅收到遗忘结果' : '共享遗忘：在线端先失效，断连端暂存旧副本'}
    </div>
    ...
    <div style={{fontSize: 36, color: '#526477', marginTop: 18}}>以上证明条目失效与查询结果，不代表磁盘副本全部物理擦除。</div>
  </div>;
};
