...
export const InsightDocumentScene: React.FC = () => {
  ...
  const focus = interpolate(f, [340, 370], [0, 1], {...clamp, easing: Easing.bezier(0.4, 0, 0.6, 1)});
  const scale = 1.2;
  ...
  return <AbsoluteFill>
    ...
    {f < SHOT_START ? <>
      <div style={{position: 'absolute', left: 135, top: 280, width: 1650, height: 525,
        ...
        <div style={{position: 'absolute', left: 30, top: 245, opacity: f >= 150 ? 1 : 0}}>
      ...
      <div style={{position: 'absolute', left: 135, top: 830, color: '#526477', fontSize: 36}}>
        {f < 150 ? '本机个人域 · 最近 24 小时 · 可继续检索的候选' : '手动写入产生候选；采集简报只统计采集日志。'}
      </div>
    </> : f < 400 ? <div style={{position: 'absolute', left: 135, top: 280, width: 1650, height: 590, overflow: 'hidden', borderRadius: 16}}>
    ...
    <div style={{position: 'absolute', left: 135, top: 910, fontSize: 36, color: '#526477'}}>
      {f < SHOT_START ? '真实截图逐行揭示 · 非模型生成文档' : f < 400 ? '真实操作录像 · 2 倍速 · 裁切放大跟随' : '真实来源正文放大 · 公开合成示例'}
    </div>
  </AbsoluteFill>;
};
