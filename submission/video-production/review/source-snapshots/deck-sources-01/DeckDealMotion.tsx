...
const DeckTexture: React.FC<{index: number}> = ({index}) => {
  ...
  return <div style={{position:'absolute',inset:0,background:'#fff',overflow:'hidden'}}>
    <Img src={staticFile(a.src)} style={{position:'absolute',width:a.imageWidth*scale,height:a.imageHeight*scale,left:-a.crop.x*scale,top:-a.crop.y*scale,clipPath:`inset(0 0 ${Math.max(0,a.imageHeight-a.crop.y-a.crop.h)*scale}px 0)`}} />
  </div>;
};
