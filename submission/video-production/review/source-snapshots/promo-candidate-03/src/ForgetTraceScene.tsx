...
export const ForgetTraceScene: React.FC = () => {
  ...
  const rejoined = frame >= 399;
  const final = frame >= 419;
  ...
  return <div style={{position: 'absolute', left: 135, right: 135, top: 170}}>
    ...
    {rows.map((row, i) => <Reveal at={(final?419:rejoined?399:284)+i*7} key={`${final}-${rejoined}-${row.device}`} style={{background: '#fff', borderBottom: '2px solid #dce2ea',
