...
export const NativeSetupScene: React.FC<{kind: 's04' | 's10' | 's11'}> = ({kind}) => {
  ...
  const stage = kind === 's04' ? (f<258?0:258) : kind==='s10' ? (f<91?0:f<358?91:358) : (f<215?0:f<359?215:359);
  return <AbsoluteFill>
    <Reveal at={stage} style={{position: 'absolute', left: 135, right: 135, top: 195}}>
      ...
      {kind==='s10' && f>=91 && f<358?<div style={{display:'flex',gap:28,marginTop:45}}><Reveal at={147} style={{flex:1,background:'#fff',padding:30,borderRadius:16}}><div style={{fontSize:42,color:'#1456b8'}}>个人范围</div><div style={{fontSize:38,marginTop:20}}>保留自己的资料</div></Reveal><Reveal at={223} style={{flex:1,background:'#fff',padding:30,borderRadius:16}}><div style={{fontSize:42,color:'#1456b8'}}>共享范围</div><Reveal at={266} style={{fontSize:38,marginTop:20}}>选择要协作的内容</Reveal></Reveal></div>:null}
    </Reveal>
    {kind==='s04'?<Steps labels={['会话与任务','记忆与设备','服务与能力']} cues={[90,145,258]} top={815}/>:kind==='s10'?<Steps labels={['录入内容','选择范围','保存并检索']} cues={[18,91,358]} top={815}/>:<Steps labels={['授权目录','文件入库','查看来源']} cues={[18,215,359]} top={815}/>}
