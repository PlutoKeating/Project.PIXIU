...
export const SPOTLIGHT_HERO_CARD_DURATION = 600; // Full action, 30f landed hold, then reveal the shared network.

const SERIF = '"Noto Sans CJK SC", sans-serif';
const INK = '#172033';
const AMBER = '#1456b8';
const SLOT_AMBER = '#1456b8';
const BEAM_CORE = 'rgba(240,247,255,0.98)';
const PATCH = '#f6f7f9';

const PAGE_H = 1200;

const CARD = {x: 781.328125, y: 616, w: 357.328125, h: 312, file: 'shared-memory.svg'};
...
const CAM_KEYS: CamKey[] = [
  ...
  { frame: 160, cx: MCX - 30, cy: MCY, zoom: 2.6, rotX: 8, rotY: 34, rotZ: 2, persp: 1200 },
  { frame: 188, cx: 960, cy: 620, zoom: 1.0, rotX: 0, rotY: 0, rotZ: 0, persp: 1200 },
  { frame: 600, cx: 960, cy: 620, zoom: 1.0, rotX: 0, rotY: 0, rotZ: 0, persp: 1200 },
];
...
export const SpotlightHeroCard: React.FC = () => {
  ...
  const lightOut = interpolate(frame, [160, 188], [1, 0], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp'});
  ...
  const shadow = `0 ${8 * lift}px ${10 + 12 * lift}px rgba(23,32,51,${0.18 * lift}), 0 ${46 * lift}px ${90 * lift}px rgba(23,32,51,${0.22 * lift})`;
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: '#f6f7f9' }}>
      <AbsoluteFill style={{ opacity: macroIn }}>
        <PageCam src="concept/shared-network.svg" crop={{x: 0, y: 0, w: 1440, h: 900}} pageH={PAGE_H} keys={CAM_KEYS} ease={PUSH_EASE}>
          ...
          <div style={{ transformStyle: 'preserve-3d' }}>
            ...
            {slotVis > 0.02 ? (
              <div
                style={{
                  ...
                  borderRadius: RADIUS,
                  boxShadow: `inset 0 0 26px rgba(20,86,184,${0.12 * slotEdge})`,
                  opacity: slotVis,
                }}
              >
                <div
            ...
            >
              ...
              >
                <Img
                  src={staticFile(`concept/${CARD.file}`)}
                  style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', display: 'block' }}
                />
                {/* 4x hi-res capture layered on top, crisp under push-in zoom */}
                <Img
                  src={staticFile('concept/shared-memory.svg')}
                  style={{
              ...
              {(beam1On || beam2On) && lift > 0.4 ? (
                <svg
                  ...
                  style={{
                    ...
                    filter: `drop-shadow(0 0 6px ${AMBER}) drop-shadow(0 0 18px rgba(220,237,255,0.55))`,
                  }}
                >
                  <rect
              ...
              <div style={{ transformStyle: 'preserve-3d', pointerEvents: 'none' }}>
                <div
                  style={{
                    ...
                    background: 'radial-gradient(ellipse at 50% 50%, rgba(23,32,51,0.3), transparent 70%)',
                ...
                >
                  <div style={{ fontFamily: SERIF, fontSize: 30, fontWeight: 600, color: INK, lineHeight: 1.16, letterSpacing: '-0.012em' }}>
                    一份记忆，
                  </div>
                  <div style={{ position: 'relative', display: 'inline-block' }}>
                    <div
                      style={{
                        ...
                        background: '#dceaff', borderRadius: 4,
                      }}
                    />
                    <div style={{ position: 'relative', fontFamily: SERIF, fontSize: 30, fontWeight: 600, color: INK, lineHeight: 1.16, letterSpacing: '-0.012em' }}>
                      多端复用。
                    </div>
                  </div>
                </div>
        ...
        <AbsoluteFill
          style={{
            background: `radial-gradient(${poolRx}px ${poolRy}px at ${spotX}% ${spotY}%, rgba(220,237,255,0.42), rgba(220,237,255,0.10) 45%, rgba(50,65,85,${vignette * spotOn}) 100%)`,
            pointerEvents: 'none', opacity: spotOn * lightOut,
          }}
        />
        <AbsoluteFill
          style={{
            background: `radial-gradient(300px 220px at ${spotX - 6}% ${spotY + 10}%, rgba(237,246,255,0.18), transparent 70%)`,
            pointerEvents: 'none', opacity: spotOn * 0.7 * lightOut,
          }}
        />
      </AbsoluteFill>
    </AbsoluteFill>
