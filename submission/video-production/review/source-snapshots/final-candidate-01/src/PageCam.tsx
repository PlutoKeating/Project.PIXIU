...
export const PageCam: React.FC<{
...
}> = ({ src, pageH, keys, children, crop = {x: 0, y: 38, w: 1440, h: 740}, blur = 0, saturate = 1, ease = Easing.bezier(0.33, 0, 0.15, 1), dof }) => {
  ...
  if (!has3D) {
    return (
      <AbsoluteFill style={{ overflow: 'hidden', backgroundColor: '#f6f7f9' }}>
        <div
          style={{
        ...
        >
          <div style={{position: "absolute", width: 1920, height: pageH, overflow: "hidden"}}><Img src={staticFile(src)} style={{position: "absolute", width: 1920 * 1440 / crop.w, height: 1920 * 900 / crop.w, left: -crop.x * 1920 / crop.w, top: -crop.y * 1920 / crop.w}} /></div>
          {children}
        </div>
      </AbsoluteFill>
    );
