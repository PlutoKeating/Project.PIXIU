...
export const SFX: SoundCue[] = [
  // SearchEvidenceScene: four letters at local 10/13/16/19, then click at 350.
  ...
  {shot: 's13', offset: peakStart(350, 0.15), src: 'audio/switch-click-quick-action.wav',
    ...
    <div style={{display: 'flex', gap: 24, flexWrap: 'wrap', justifyContent: 'center'}}>
      {panel.cards.map(([title, detail], index) => {
        ...
        return <div key={title} style={{background: '#fff', border: '1px solid #dce2ea', borderRadius: 20,
          ...
          <div style={{fontSize: 38, lineHeight: 1.65}}>{detail}</div>
        </div>;
      })}
    </div>
    <div style={{fontSize: 38, color: MUTED, textAlign: 'center', marginTop: 46}}>{panel.note}</div>
  </AbsoluteFill>;
};
...
export const ShotScene: React.FC<{shot: Shot; includeAudio?: boolean; includeCaptions?: boolean;
  ...
  return <AbsoluteFill style={{background: '#f6f7f9', color: INK, fontFamily: '"Noto Sans CJK SC", sans-serif'}}>
    {shot.id === 's30' ? <SceneOutroLive duration={shot.duration} />
      ...
      : shot.id === 's24' || shot.id === 's25' || shot.id === 's27' ? <NativeDeliveryScene kind={shot.id} />
