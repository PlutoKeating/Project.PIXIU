    ...
    <div style={{position: 'absolute', left: 135, right: 135, top: 195}}>
      {kind === 's24' ? <>
        ...
        <div style={{fontSize: 38, color: '#1456b8', marginBottom: 35}}>
          {f < 184 ? '演示安装记录 · 0.1.9' : f < 241 ? '设置 · 系统云模型检测结果' : '设置 · 本次更新检查结果'}
        </div>
        {f < 184 ? <>
        ...
        </> : f < 241 ? <Crop src="54-model-check.png" x={478} y={323} w={425} h={156} />
          : <>
            <Crop src="51-update-check.png" x={502} y={300} w={411} h={162} />
            <div style={{fontSize: 36, color: '#526477', marginTop: 38}}>本镜展示检查结果；未执行一次版本升级。</div>
          </>}
      </>}
    </div>
    <div style={{position: 'absolute', left: 135, top: 905, fontSize: 36, color: '#526477'}}>
      {kind === 's27' ? '原生页面与终端裁片 · 演示安装版本，最终候选待重建验证' : '原生页面裁片与功能说明 · 公开合成便笺'}
    </div>
  </AbsoluteFill>;
};
