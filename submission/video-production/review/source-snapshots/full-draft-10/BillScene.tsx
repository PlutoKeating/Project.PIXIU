...
const rows = [
  {x: 13, y: 4, w: 1515, h: 87, bg: '#f6f7f9'},
  {x: 13, y: 105, w: 1515, h: 53, bg: '#f6f7f9'},
  {x: 42, y: 250, w: 1480, h: 63, bg: '#ffffff'},
];
const EvidenceRows: React.FC = () => {
  const frame = useCurrentFrame();
  const pan = interpolate(frame, [0, 68], [24, 0], clamp);
  return <div style={{position: 'absolute', left: 170, top: 270 + pan, width: 1580, height: 550}}>
    <Img src={evidenceSrc} style={{width: 1580, height: 550}} />
    {rows.map((r, i) => {
      ...
      return <React.Fragment key={i}>
        {patchOpacity > 0 && <div style={{position: 'absolute', left: r.x, top: r.y,
          width: r.w, height: r.h, background: r.bg, opacity: patchOpacity}} />}
        {frame >= cue && frame < land + 4 && <div style={{position: 'absolute',
          left: r.x, top: r.y, width: r.w, height: r.h, borderRadius: 8,
        ...
        {frame >= land && frame < land + 8 && <div style={{position: 'absolute', left: r.x,
          top: r.y, width: r.w, height: r.h, borderRadius: 8, overflow: 'hidden', zIndex: 4}}>
...
export const BillScene: React.FC<{evidence?: boolean}> = ({evidence = false}) => {
  ...
  return <>
    {source ? <Sequence from={97}><EvidenceRows /></Sequence> : <Img src={staticFile(`clips/${file}`)}
    ...
    <div style={{position: 'absolute', left: 140, bottom: 140, color: '#526477', fontSize: 23}}>
      {source ? '真实证据详情 · 截图逐行强调 · 来源列表不等于逐句引用'
