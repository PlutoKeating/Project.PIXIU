...
import { DeckCamera, CamKey2D } from './DeckCamera';
import assets from './deck-assets.json';

export const DECK_DEAL_FLYIN_DURATION = 113;

const cards = assets.map((asset, i) => ({file: i, x: [408,781.328125,1154.65625][i%3], y: 223+393*Math.floor(i/3), w:357.328125, h:312, title: ''}));
const PAGE_H = 1800;
const PAPER = '#f6f7f9';
...
const DeckTexture: React.FC<{index: number}> = ({index}) => {
  const a=assets[index]; const scale=CARD_W/a.crop.w;
  return <div style={{position:'absolute',inset:0,background:'#fff',overflow:'hidden'}}>
    <Img src={staticFile(a.src)} style={{position:'absolute',width:a.imageWidth*scale,height:a.imageHeight*scale,opacity:0.42,filter:'blur(1.2px)',left:-a.crop.x*scale,top:-a.crop.y*scale,clipPath:`inset(0 0 ${Math.max(0,a.imageHeight-a.crop.y-a.crop.h)*scale}px 0)`}} />
  </div>;
};
...
export const DeckDealMotion: React.FC = () => {
  ...
  return (
    <DeckCamera src="textures/live/projects-empty.png" pageH={PAGE_H} keys={CAM_KEYS}>
      ...
      {frame < METAL_FADE[1] ? (
        <div
          style={{
            ...
            background: [
              `radial-gradient(1300px 900px at ${3000 + PILE_CX}px ${3000 + PILE_CY}px, rgba(20,86,184,0.12), rgba(20,86,184,0.03) 40%, transparent 68%)`,
              'repeating-linear-gradient(100deg, rgba(255,255,255,0.028) 0px, rgba(255,255,255,0.028) 1px, transparent 2px, transparent 7px)',
              'repeating-linear-gradient(100deg, rgba(20,86,184,0.025) 0px, rgba(20,86,184,0.025) 2px, transparent 4px, transparent 13px)',
              'linear-gradient(115deg, #e1e7ef 0%, #f6f7f9 28%, #dce2ea 55%, #eef2f7 78%, #e2e8f0 100%)',
            ].join(', '),
            pointerEvents: 'none',
          }}
        />
      ...
      <div
        style={{
          ...
          width: PAPER_EXT.w, height: PAPER_EXT.h, background: PAPER, transform: 'translateZ(-12px)', pointerEvents: 'none',
        }}
      />
      ...
      {grid.map((c, i) => {
        ...
        const transform = landed
          ? (frame >= 101 ? 'none' : 'translate3d(0px, 0px, 0px)')
          : inPile
        ...
        const shadow = landed
          ? '0 2px 6px rgba(23,32,51,.08)'
          : inPile
            ? '0 1px 3px rgba(23,32,51,.14)'
            : `0 ${36 - 30 * settleT}px ${70 - 60 * settleT}px rgba(23,32,51,${0.3 - 0.22 * settleT})`;
        ...
        return (
          <div key={`${c.file}-${i}`} style={{ transformStyle: frame >= 101 ? 'flat' : 'preserve-3d' }}>
            {showGhost ? (
              <div
                style={{
