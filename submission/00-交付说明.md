...
打开本目录即可直接看到交付文件，不再经过 `review/` 或空的 `final/` 中间层：

- `00-交付说明.md`：本说明、当前状态和替换要求；
- `01-项目报告/`～`05-功能演示视频/`：PPT 第 21 页规定的五类交付物；
- `build/release/submission_tools/`：机器清单、文档渲染、证据归档、源码归档、最终
  校验和测试工具。工程工具不属于比赛交付物，因此不放在本目录中。

`build/release/evidence/` 仍是候选供应链实物的受忽略工作目录；其内容不直接进入
Git，但正式 D-03 归档器会复制必要证据并纳入逐文件摘要。
...
当前目录严格按 PPT 第 21 页的五项交付物编号；文字方案要求的补充文档归入
...
python3 build/release/submission_tools/render_documents.py --draft-output DRAFT_DIRECTORY
python3 build/release/submission_tools/render_documents.py --render-final
python3 build/release/submission_tools/build_evidence_archive.py --build \
...
python3 build/release/submission_tools/build_source_archive.py \
  --root . --evidence-dir AGENT_EVIDENCE \
  --output "submission/03-源代码及规范/Project.PIXIU-source.tar.gz"
python3 build/release/submission_tools/build_submission.py --check
python3 build/release/submission_tools/build_submission.py --package
```

`--draft-output` 必须指向 `submission/` 之外的临时审阅目录，所有页面自动标注
...
`--package` 从本目录的五类文件创建固定外层名称 ZIP，并输出到 `build/release/out/`。
任何门未通过、文件缺失、命名/格式错误、官方原件哈希改变或工作区不
