...
最终出包还必须反查 PPT 第 21 页七个踩雷项：端侧性能/资源、关联检索、规范数据集、
架构与技术栈表达、非纯文字/txt、银河麒麟 V11 实现、非开源软件原样提交。详细证据
口径统一引用 `docs/AcceptanceTestSpecification.md` 的 T-01～T-07，不在本目录另造标准。
...
- `render_documents.py`：把 D-02/D-03/D-04 和 D-02 附件 A-01～A-05 的 Markdown 权威源排版为
...
- `final/`：最终文件生成位置，目录与正式生成物均进入 Git；发布门未全部通过前只保留说明文件，避免把占位物或审阅稿冒充正式结果；
- `review/`：本机可重复生成的审阅实物、人工材料占位物与演示素材，全部进入 Git；不得复制到 `final/` 冒充已验收文件；
...
生命周期闭环、最终 V11 指标、
三设备同步、安装/升级和 D-01～D-05、A-01～A-05 全部以同一 commit 通过并审核后，才可填写最终
commit、把各门设置为 `passed`。

截至当前，仓库能够自行维护和生成 D-02、D-03、D-04、A-01～A-05 及 R-01
相关安装资产；在全部正式门关闭前只能生成带水印审阅稿。团队负责人
最终需要提供的外部材料严格收敛为 D-01 项目报告 PPT、D-05 ≤7 分钟演示视频、盖章
报名表，并对自动生成文档签字审核。确定性 Mock 只服务工程调试，不能作为缺失的
`agent_lifecycle` 正式主记录放入原始证据 ZIP。

当前审阅目录严格按 PPT 第 21 页的五项交付物编号；文字方案要求的补充文档归入
第 2 类，安装软件归入第 4 类，Mock 素材归入第 5 类的独立子目录：

| 子目录 | 内容 | 状态 |
|--------|------|------|
| `01-项目报告/` | `PIXIU项目报告.pptx` 零字节人工占位物 + 编写说明 | 必须由负责人替换，不得提交占位物 |
| `02-技术方案及测试结果/` | DOCX + PDF；内含用户手册、效果与测试证据、记忆流转、实际案例、V11 适配五类附件 | 带草稿标识；七类正式原始证据未齐 |
| `03-源代码及规范/` | 含四个 submodule 实体的源码审阅包 + 许可证 PDF | 可审阅；冻结后改为正式文件名 |
| `04-部署文档/` | 部署指南 PDF；`01-可安装软件/` 内含 `.deb` 与 SHA-256 | 可审阅；正式签名和最终同版矩阵待补 |
| `05-功能演示视频/` | `PIXIU功能演示.mp4` 零字节人工占位物；`01-Mock演示素材/` 保存中文重录清单 | 旧素材因维护模式叠层和脱敏问题已移除；须干净重录，且 Mock 不替代 D-05 |

整个 `submission/` 都由 Git 跟踪；二进制交付物使用仓库根目录 `.gitattributes`
声明的 Git LFS 规则管理。克隆后应先安装 Git LFS 并运行 `git lfs pull`，否则工作区中
只会得到 LFS 指针文件。
...
洁净时都会失败。A-02 的 `three-device-final-suite.json` 还必须满足最终三设备契约，
