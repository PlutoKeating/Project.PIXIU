...
rule = re.search(r'QTabWidget#pixiuWorkspaces QListWidget::item:selected, QFileDialog QAbstractItemView::item:selected\s*\{([^}]+)\}', source)
...
PY
grep -q 'qtWidgetsTranslation.load(QLocale::system()' "${fixture}/source/src/main.cpp"
grep -q 'app.installTranslator(&qtWidgetsTranslation)' "${fixture}/source/src/main.cpp"
