...
cd "$root"
case "${PIXIU_KYSDK:-ON}" in
    ON) target_os=kylin-v11 ;;
    OFF) target_os=generic-debian ;;
    *) echo "PIXIU_KYSDK must be ON or OFF" >&2; exit 2 ;;
esac
case "${1:-}" in
    --describe) printf '%s\n' "$target_os"; exit 0 ;;
    '') ;;
    *) echo "Usage: prepare-agent-supply-chain.sh [--describe]" >&2; exit 2 ;;
esac
# Fail before creating evidence when the committed Runtime lock cannot apply.
test "$(dpkg --print-architecture)" = amd64 || {
    echo "The committed Agent Runtime lock requires amd64" >&2; exit 2
}
test "$(python3 -c 'import sys; print(sys.implementation.cache_tag)')" = cpython-312 || {
    echo "The committed Agent Runtime lock requires CPython 3.12" >&2; exit 2
}
...
sudo -n unshare --net runuser -u "$builder" -- \
    env PATH="$PATH" PIXIU_KYSDK="${PIXIU_KYSDK:-ON}" \
    TMPDIR="${TMPDIR:-/tmp}" CMAKE_BUILD_PARALLEL_LEVEL=2 \
...
sudo -n unshare --net runuser -u "$builder" -- \
    env PATH="$PATH" TMPDIR="${TMPDIR:-/tmp}" \
...
"${record[@]}" runtime-wheelhouse --target-arch amd64 --python-abi cp312 --target-os "$target_os" \
