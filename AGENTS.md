...
> 2026-09-06 仓库现状：远程默认分支为 `main`，现有个人模块分支；尚无 `staging`/`production` 远程分支。下文三阶段流是协作规范，不代表这些分支已创建。当前自动发布入口是 `main` 上版本提交对应的 `v*` 标签，经通用 CI 与 V11 原生门后发布；这不改变逐次取得 Human 推送/发布授权的要求。
