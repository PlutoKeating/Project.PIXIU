...
- 🟡 **当前总览**：组合回归 809 passed、前端 ctest 38/38；版本化 `.deb`、签名
  ...
  真实 Agent 两阶段采集器及 8 项契约测试已落地，要求三轮、双进程重启、Shell/联网/
  记忆工具、现场审批和跨会话召回；当前仍缺最终 V11 实测、最终性能和安装矩阵等输入，
  因此按设计不能生成最终 ZIP。
  最终性能汇总器已落地，强制四项指标原始样本、三组各 30 例消融和
  strict/Agent/数据集/候选包同版；三变体真实 Agent 采集器及 6 项契约测试也已补齐，
  核对全记忆关闭、strict 单机、三机不同节点分布式画像，逐任务复算且不保留模型正文；
  现有 portable 报告按设计不能通过该门。
  ...
  检查点，供应链递归到构建/源码/wheel/锁文件/日志/SBOM/NOTICE；安装矩阵
