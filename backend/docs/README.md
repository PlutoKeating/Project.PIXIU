...
- **去中心化记忆同步** — 这是 PIXIU 的标志性能力：多台麒麟设备对等组网、无中心服务器。设备配对后自动建立信任，本地写入的记忆经 **Gossip 实时推送 + 反熵周期对账** 扩散到全网，用 **CRDT** 自动合并并发修改；断网各自照常用，重连自动对齐；私有与高敏数据默认不出端。完整管理链路（节点身份、配对、发现、对账、删除传播、权限管控）见 [`ARCHITECTURE.md` 第 7 章](./ARCHITECTURE.md#7-分布式同步层pixiusync-去中心化多节点记忆共享管理链路)。
- **量化评测框架** — 一套可复现的脚本，验证偏好提取准确率、检索召回率、延迟、冲突处理正确率等指标。

> **同步 vs 冲突仲裁的区别**：去中心化同步（CRDT）解决"同一条记忆在多设备并发修改"的**数据层**收敛；M4 冲突仲裁解决"不同记忆在事实上互相矛盾"的**知识层**裁决。二者正交，详见架构文档。
