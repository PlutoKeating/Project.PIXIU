...
核心模型拒绝。目录监视器已传入该字段，正式界面展示尚待接入，旧调用默认 `None`；
...
删除前拒绝。前置检查后调用仓储 `forget_if_versions` 原子校验整批 ACTIVE 目标，
同时标记遗忘并递增版本，拒绝检查后版本漂移；不支持该契约的仓储明确失败。
HTTP API 已接入预览凭证，向量与同步副作用恢复仍需闭环。未传此参数的旧调用尚保持原有行为，不能视为
...
适配器同时暴露 `LoadDBFile` 与显式 `Disconnect`；Module C 组合根在 strict 启动预检
时先装载 `PIXIU_VECTOR_DB_PATH`，并按进程复用一个 store、退出时断开。空路径及启用
加密但缺少密钥会在进入 SDK 前拒绝。
...
class KnowledgeService:
    ...
    async def materialize(self, item: KnowledgeItem) -> KnowledgeItem: ...  # 远端可信结构化项重建图/向量
    async def forget(self, knowledge_id: str) -> None: ...  # 远端墓碑隐藏并删除向量
