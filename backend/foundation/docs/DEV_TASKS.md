...
目录监视已统一接入受控 dreaming；旧同进程直接入库桥接和知识 OCR 分支已删除。监视器测试覆盖真实 watchdog 的配置、防抖、分发和异常隔离；文档与记忆内容验证由文档 API 和 dreaming 测试负责。
...
2026-09-10：新增 documents 解码、登记及分块 API，schema v14 增加私有暂存表。解码/登记/schema 39 项、API 1 项通过；后端全套 933 项通过。正式目录装配现使用 dreaming；旧 IngestBridge 已删除。真实模型目录运行尚未验证。
...
2026-09-10：同包双设备并发验收发现正文一致但来源残留失败分支。仓储 save 现按完整快照事务性替换来源关系；同步物化清除旧分支待补来源，原证据不删除。新增回归覆盖快照替换、空来源与迟到失败分支来源；Foundation/Engine 共 894 项通过。无需新依赖、schema 或打包路径，现有递归打包覆盖两处修复；新版安装包复验待完成。
