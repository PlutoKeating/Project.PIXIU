...
2026-09-10：新增 documents 解码、登记及分块 API，schema v14 增加私有暂存表。解码/登记/schema 39 项、API 1 项通过；后端全套 933 项通过。正式目录装配现使用 dreaming；原 IngestBridge 同进程入口保留现有测试调用。真实模型目录运行尚未验证。
