...
`mcp/` 使用 SDK 1.26.0，通过公共文档 API 提供任务授权范围内的真实 stdio 工具；`dreaming/session.py` 提供固定私人范围的检索、来源校验、计划及获准创建，向量更新沿既有 `/memory/write` 进行。目录已接 Runtime 认证入口与逐块模型工具循环，聊天 MCP 入口尚未接线。模拟模型链路通过不代表真实模型或最终包已交付。构建包携带这两个模块，离线依赖锁仍需更新验证。
