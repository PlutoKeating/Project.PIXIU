...
- **变化提示**：正式宿主对采集事件提示受影响页面，刷新可见日志并使旧洞察/
  简报失效；不把文件名与标题字串重叠解释为语义关联。旧相关性推断、每日计数
  和含文件名/业务标题的通知路径已删除。偏好提醒基于成功读取列表的范围、ID
  和版本变化，以固定文案限流提示，不宣称实时学习或完整偏好统计。
...
| Agent 产品身份 | 同包 `SOUL.md` + 宿主启动写入 | PIXIU 本地 Working Agent、工具工作流与分布式记忆工作台 | backend/agent + build/release/agent-host | 启动前事务部署并由 Gateway 读取 |
...
图片知识沿多模态模型理解 → 用户核对 → 证据/知识写入 → 检索来源运行。原图放在证据原始字段中随既有共享/遗忘处理，不复制到知识正文和向量输入。

日常交互采用会话优先、服务自动维护、实质操作审批。偏好由 `/memory/write` 自动提取，管理界面只读取与处理需确认的冲突；无需手动触发提取或刷新。正常连接不显示技术横幅，后台工作报告按任务出现。界面简化与服务对应见 `docs/PRODUCT_EXPERIENCE_SIMPLIFICATION.md`。
