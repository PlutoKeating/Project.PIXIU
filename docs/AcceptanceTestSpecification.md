...
SDK 边界，但仍须在纳入完整 Agent 的同一最终候选上生成正式 A-02 原始证据。
...
| A-10a `[A][D]` | 宿主 UI 可用性（团队质量门） | 默认浅色；采用稳重科技蓝语义色板；设置页“浅色 / 深色”控件点击后即时生效并持久化；V11 浅/深主题下文字、占位符、下拉框、按钮与消息均可读；关键普通文本对比度 ≥4.5:1；Enter 发送，Shift+Enter/Ctrl+Enter 换行；提交后立即显示等待气泡；每轮 LLM 文本保留为独立消息，工具/记忆/Web 搜索的 running/completed 过程在默认折叠的动态卡片中连续更新；空状态不泄露内部路径；100%/125%/150% 缩放与键盘焦点可用；刷新历史、折叠侧栏或缩放后每条消息恰好一个气泡，无错位残影或空状态遮挡；长会话标题不产生横向滚动或挤压主区；宽屏消息保持在居中阅读列内连续排列，短消息不被压成逐字窄列，末条消息不被输入区裁切 |
...
模型自主性证据，也不得计入正式 A-02 的 `kylin-v11-agent-lifecycle` 主记录。
...
PPT 第 21 页是交付物顶层分类的直接依据，正式提交目录必须且只能按以下五类编号。
文字方案要求的用户手册、实际案例、适配报告和更细的测试材料不另造第六至第十个
官方顶层类别，而是作为“02 技术方案及测试结果”的内部附件共同交付。
...
### 7.1 D-02 内部必备附件（非新增官方顶层类别）

| 内部编号 | 附件 | 必须内容/格式 |
|----------|------|---------------|
| A-01 | 用户手册 | 完整操作、权限、隐私、同步、遗忘、错误恢复 |
| A-02 | 效果/测试报告与原始证据 | 数据集说明、对比实验、量化指标、原始结果、环境和失败分析 |
| A-03 | 记忆流转说明 | 短期、中期、长期的边界、交互、晋升、清理与 Agent 生命周期 |
| A-04 | 实际应用案例 | 至少 1 个真实场景；建议同时覆盖附录 A 家庭支出和多设备协作 |
| A-05 | V11 适配报告 | 系统信息、硬门槛证据、资源占用、兼容性与问题记录 |
...
摘要、验证 Ed25519 签名，改扩展名或空壳文件不得计为 D-01～D-05/A-01～A-05/R-01 通过。
A-02 `原始证据.zip` 还有内容级门禁：双 SDK、完整 Agent、性能、三设备、安装升级、
...
6. N-01～N-08 多设备创新专项。
7. D-01～D-05、D-02 附件 A-01～A-05 与团队 R-01～R-06 交付复核，按 S1/S2 独立评分。
