...
> 为准；本规范其余技术验收条目继续适用。
>
> 依据：[官方技术/赛事细则 PPTX](完整赛题要求.pptx)、[官方平台赛题文字方案与附录 A](OriginProblemDescription.md)；技术要求共同适用，交付要求见冻结记录
...
SDK 边界，但仍须在纳入完整 Agent 的同一最终候选上生成内部原始验证记录。
...
规范化数据摘要、实际 JSON 摘要、test case ID 摘要和赛题记录摘要。生成器要求
...
唯一依据为[赛方交付作品要求冻结记录](DELIVERY_PLAN.md#0-赛方交付作品要求唯一权威永久冻结)。
...
按冻结记录的两层同名目录组织，源代码目录与内层材料目录并列，命名信息与平台一致。
作品内容一律不得出现学校、学院、校徽、指导教师、参赛学生姓名等可识别身份信息。
用户手册、安装部署、测试及效果验证报告属于技术方案内容，不另设必交件。
...
测试结论须有真实、可追溯的内部数据支持；量化评测与效果分析纳入技术方案。
内部原始证据不另列为必交压缩包。
...
交付目录、命名与匿名要求按冻结记录执行；内部版本、测试与审核记录支撑材料结论。
