...
| H-02 `[GATE]` | 数据库使用系统向量数据库 SDK | 跟踪建库、索引、写入、删除、查询调用；检查链接/进程与运行日志 | 生产向量存储和检索实际调用 `kylin-ai-vector-engine`/官方客户端；严格模式缺失即失败 | **不通过：strict 生产适配已在 V11 编译成包；系统服务 UID 无法访问桌面用户 AI runtime socket，真服务生命周期证据待补** |
