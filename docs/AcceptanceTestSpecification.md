...
>
> 依据：[完整赛题要求 PPTX](完整赛题要求.pptx)、[赛题文字方案与附录 A](OriginProblemDescription.md)
...
PPT 对 OS Agent 的口径是“会用工具、会规划、会记忆，不只是聊天”。以下是证明记忆方案已基于完整可用 OS Agent 的派生验收，不擅自把它们升级为 PPT 的零分条款。
...
推荐验收基座是 `third_party/kylin-agent` + `third_party/kylin-agent-runtime`，通过 MemoryProvider 适配 PIXIU；不要求从零重写这些通用 Agent 能力。
...
PPT 的初次作品提交要求与文字方案要求合并执行；格式冲突时采取更严格口径。
...
文字方案写 2026-09-15 前提交、9 月底初审、11 月终审；2026.05 PPT 写 10 月初提交、10 月评审、11～12 月总决赛。两者冲突。项目文档统一标记为“待赛方最新正式通知确认”，不得据任一内部文档自行承诺最终截止时间。
