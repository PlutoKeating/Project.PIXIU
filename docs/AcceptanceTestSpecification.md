...
D-07 内容门禁对 native SDK、Agent 生命周期、性能和数据集执行各自深度验证器，而非
...
50/15/25 用例、90 个唯一 case ID、20 次重复和规范化摘要。任一步失败，归档生成和
最终解包复验都失败。
...
这些只覆盖 R-03～R-05 的部分切片；完整安装
矩阵和最终 V11 图形升级/重启证据仍未完成，R-03～R-05 继续判未通过。
