...
| A-10a `[A][D]` | 宿主 UI 可用性（团队质量门） | 默认浅色；V11 浅/深主题下文字、占位符、下拉框、按钮与消息均可读；关键普通文本对比度 ≥4.5:1；Enter 发送，Shift+Enter/Ctrl+Enter 换行；提交后立即显示等待气泡；每轮 LLM 文本保留为独立消息，工具/记忆/Web 搜索的 running/completed 过程在默认折叠的动态卡片中连续更新；空状态不泄露内部路径；100%/125%/150% 缩放与键盘焦点可用；刷新历史、折叠侧栏或缩放后每条消息恰好一个气泡，无错位残影或空状态遮挡；长会话标题不产生横向滚动或挤压主区；宽屏消息保持在居中阅读列内连续排列，短消息不被压成逐字窄列，末条消息不被输入区裁切 |
