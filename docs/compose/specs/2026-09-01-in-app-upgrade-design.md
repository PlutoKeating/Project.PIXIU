...
### [S2.1] 远程版本检测
- `GET https://api.github.com/repos/PlutoKeating/Project.PIXIU/releases/latest`（公开 repo，匿名可达）→ 解析 `tag_name`（如 `v0.1.6`）+ `assets`（找 `pixiu_<ver>-1_amd64.deb` 与 `.sha256` 的 `browser_download_url`）。
...
- 下载 `.sha256` 文件（release 随包），本地 `sha256sum <deb>` 重算与发布 digest 比对——不一致则中止删除并报「校验失败，已中止」，防止下载到损坏/篡改包。
- 下载完成时校验通过后才进入安装步骤。

### [S2.3] 特权安装（pkexec）
- `QProcess::startDetached("pkexec", {"dpkg", "-i", debPath})` 或 `start("pkexec", {"dpkg", "-i", debPath})`——pkexec 触发 polkit 认证框（root 授权安装）。
...
- 下载取消/断网中途——清理临时文件，状态回「已取消/失败」，可重试。
