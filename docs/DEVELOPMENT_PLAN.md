...
6. 以 ADR-0004 的受控下游补丁实现默认浅色、Ant Design 风格科技蓝双主题、设置页即时切换和可访问性；旧界面截图
...
8. Agent 消息使用包内离线 Markdown/KaTeX/Mermaid/emoji 渲染；工具、记忆、Shell、
   Web 搜索等工作事件以默认折叠的动态卡片实时更新并随会话持久化，卡片元数据不进入模型上下文；
   消息采用自然词距，发送、流式分段、工作事件与异步富文本高度变化统一跟随最新位置，工作卡片
   与助手消息共享左侧内容列。模型输入中的系统提示、工具说明和助手历史统一使用 PIXIU 产品身份。
...
| 6 | 单一安装包与版本/升级闭环 | A + C + E + D | V11 图形安装；GUI 签名升级、健康检查、恢复和数据保留通过；Agent 宿主默认浅色、设置页双主题切换、PIXIU 正向身份、离线富消息渲染与完整 SSE 工作流可见 |
| 7 | 最终文档与答辩 | D + Human | 项目报告、技术方案、功能演示视频、源代码四项作品同版且匿名审核完成 |
...
单一 `.deb`、GUI 一键升级、版本/兼容矩阵、恢复策略和 四项作品 文档状态统一
...
| 最终交付计划 | `docs/DELIVERY_PLAN.md` | 单一安装包、版本/升级与 四项作品 台账 |
