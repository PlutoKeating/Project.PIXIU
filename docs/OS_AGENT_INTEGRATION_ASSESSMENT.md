...
常见失分点。交付条款已按用户提供的新图片更新，唯一执行口径见 `DELIVERY_PLAN.md` §0。
...
这些 submodule 是上游权威参考。除非形成明确的补丁维护策略，不直接修改 submodule
工作树。当前固定版本中，`kylin-agent` 的官方 Gitee API 元数据声明 `AGPL-3.0`，按
SPDX 3.0 规范化为 `AGPL-3.0-only`；`agent-runtime` 为 MIT。清单保留元数据 URL、原值
和规范化结果。打包或派生分发仍必须提供完整对应源码、许可证、版权声明、NOTICE、
依赖清单并完成供应链审查。任何上游脚本也必须在执行前审阅，不能盲跑。
...
| 把系统已安装的 openKylin Agent 当宿主，通过独立适配器调用 PIXIU | 官方材料未禁止，作品主体仍是原创记忆系统 | **已批准执行**；保留架构、接口、代码量与贡献证明 |
| fork Agent 并大范围改写后整体提交 | 未被明文禁止，但原创边界、AGPL、维护和答辩风险更高 | 仅在扩展接口无法满足时采用最小补丁 |
| 团队从零开发完整 Agent | 未被要求，且会稀释记忆创新与时间 | 不采用 |
...
`agent-runtime` 已提供单一外部 `MemoryProvider` 生命周期接口，团队批准将其作为接缝：
...
验收采集不得根据提示词自述“调用成功”。当前工具要求场景不出现实现工具名，通过
上游 Gateway 事件观察 `terminal`、`web_search` 和 PIXIU 记忆工具；随机标记先进入
临时受控文件，只有 Agent 消费 Shell 结果并选择记忆写入后，重启后的新会话才可能
召回。最终报告仅保留摘要和计数，真实提示、结果及身份不进入交付归档。
...
1. **P0 合规闭环**：把已通过的 V11 双 SDK 产品探针固化为最终 user service、安装依赖与取证；严格模式不允许静默降级。
2. **P0 Agent 供应链**：落实 ADR-0003，交付可重建宿主、固定离线 Runtime 和许可证/SBOM；禁止首次启动漂移版本。
3. **P0 Agent 闭环**：以已实现 MemoryProvider 跑通多轮会话→自主召回→工具执行→工具结果写入→后续轮次复用。
4. **P0 证据闭环**：分别产出 Debian portable 回归与 V11 `KYSDK=ON` 最终验收报告。
5. **P1 分布式亮点**：至少三台设备验证并发写、离线、重连、冲突、遗忘墓碑与最终收敛。
6. **P1 评测与答辩**：用合法、标准化数据集覆盖偏好、召回、延迟、冲突和 Agent 端到端自主性。
...
V11、双 SDK 和踩雷提示继续适用；作品内容、份数、格式、目录和匿名要求统一按 `DELIVERY_PLAN.md` §0 执行。

2026-09-10：超过两个文本块的纯文字附件不再一次拼入模型输入；保留文档暂存引用（既有 24 小时有效期），初始消息给出读取游标。Provider 新增只读 pixiu_document_read，按随机 document_id、原文件 version 和 cursor 读取单块文字并返回 next_cursor；不接受路径，不写记忆。模型需读至末块才可声称全文读取，后续问题可回读。短附件沿用现有直送；图片密集文档现已接入分批视觉工具，真实模型容量验证尚未完成。无新增依赖/构建文件，现有 Provider 与 Runtime 源码打包覆盖变更。26 项 Provider/附件/MCP 测试通过。
