...
| Embedding | 当前审阅候选 strict 单包同版原生证据经 runtime 1.3.0 使用 gte-base 768 维真实向量 | 技术门通过；最终冻结 commit 需重跑并进入 D-07 |
| 向量数据库 | 当前审阅候选 strict 单包完成 direct SDK 和产品写入/检索/遗忘/隐藏 | 技术门通过；最终冻结 commit 需重跑并进入 D-07 |
