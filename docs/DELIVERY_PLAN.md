...
> `完整赛题要求.pptx` 保持历史原件不变；其他赛题技术要求继续适用。
>
> 实施顺序、依赖和逐切片完成定义见
...
### 0.5 生效范围与历史冲突（记录说明，非赛方原文）

- 按用户本次明确指令，§0.1～§0.3 取代历史材料及派生文档中交付作品的
  内容、份数、格式、命名与匿名口径；历史原件只读保留，不再作为相冲突条款的执行依据。
- 旧“视频不超过 7 分钟”改以 **5-10 分钟、zip 压缩包、大小不超过 200M** 为准。
  `200M` 按图片原文留存，不擅自补充字节换算；图片未指定压缩包内视频编码或扩展名。
- 图片一写明技术方案为 word、pdf 文档，图片二示例名为 `技术方案.doc`；
  两者均完整保留，不把示例擅自改成 `.docx`，也不额外规定 Word 与 PDF 必须各交一份。
- 旧“学校全称－选题名称－作品名称”及仓库 `01～05` 五类目录只作历史/内部组织方式；
  最终作品目录以 §0.2 为准。安装部署指南、用户手册和效果验证报告按 §0.1 纳入技术方案，
  不据历史内部编号新增赛方顶层交付类别。
- 目录命名需要身份信息与作品内容必须匿名分别适用，不能用命名要求豁免正文匿名。
  内部作者/审核人实名记录不进入提交作品内容。
- 本次只记录要求和团队信息，不调整提交实物、生成材料、修改打包脚本或执行提交。
  既有其他文档与工具若仍保留旧口径，不代表其优先于本节，也不代表已完成新要求适配。
...
| D-05 | 演示视频 | `delivery/PRESENTATION_AND_VIDEO.md` | 按 §0.1：5-10 分钟，zip，大小不超过 200M | 脚本为旧口径，录制待完成 |
...
“部分完成”不等于可提交。内部审核记录保存作者/审核人；提交文档按 §0.3 匿名，
保存版本、日期、适用提交、环境、证据链接和已知限制；数据和截图必须能追溯到
同一个 release commit。上表编号和维护格式为内部台账，最终份数、格式与目录以 §0 为准。
...
最终目录命名与层级按 §0.2、§0.4，视频按 §0.1；旧外层目录/ZIP 命名不再适用。
历史打包流程所述盖章报名表属于申报手续材料，不应据此放入匿名作品正文。
视频/PPT 不由
