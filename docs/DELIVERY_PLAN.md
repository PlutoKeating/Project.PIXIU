...
| 功能演示视频 | 1 份，5-10 分钟，`演示视频.zip`，大小不超过 200M | `delivery/PRESENTATION_AND_VIDEO.md`、`submission/video-production/` | 第十四版中文初剪已生成，9 分 21 秒、约 56.2 MB；同步检查点、历史量化表、高清偏好堆叠、非标题搜索、洞察来源链、跨端遗忘检查点、设备信任及共享边界控制已入片，整片解码通过；洞察日期字号与来源时序通过有限复核，仍有功能/模板细节/视听余项，正式 ZIP 待完成 |
| 源代码 | 1 份，完整源代码及必要技术规范要素 | `delivery/SOURCE_AND_LICENSES.md` 与项目源码 | 按当前源码及四个固定上游生成，逐文件核对 |

提交作品匿名；团队信息只用于平台信息核对与两层目录命名。内部保留版本和审核追踪。

视频行为来源补录暂未取得自动落库证据，不能作为已完成功能演示。失败素材与
隔离环境恢复检查已保存在制作目录；该缺口仍属最终视频功能覆盖余项。
跨端遗忘已新增章节样片，按原始接口记录呈现断连保留、重连失效及三端删除标记一致；
末段证据范围修正已通过有限独立复核并接入第十三版全片，不作最终视频放行。
设备页暂停/恢复已补原生4K录屏与状态回读，配对入口和解除确认仅取得入口素材，
尚未接入全片，不将入口截图当作配对或解除完成证据。
后续补录已确认一侧解除与原生配对恢复结果，双方节点集合恢复；设备章节新增
真实裁片样片已通过有限独立复核并接入第十四版全片，不能据此宣称本次已验证实际数据传输。
