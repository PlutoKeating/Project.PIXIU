...
| V11 双 SDK 严格画像 | strict 构建已通过；官方修复组合下 PIXIU binding 已返回 768 维真实向量；产品探针发现并已在源码修复 Vector `LoadDBFile` 漏接，用户会话/最终包仍待验 | 阻断 |
...
进程复用并在退出时断开。该修复已通过 790 项组合测试；只有重建的同版 V11 候选
完成写入、召回、遗忘和隐藏检查后，才可更新本表状态。
