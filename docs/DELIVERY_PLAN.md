...
当前正式产品版本为 [0.1.12](https://github.com/PlutoKeating/Project.PIXIU/releases/tag/v0.1.12)（2026-09-10 已发布）：单一 openKylin Agent 桌面宿主及内嵌记忆、设备、设置
...
全片独立有限终检已完成30镜335帧，剩余关键小字、两类偏好结果、动作音效和正式
审阅标记四类必要修订；30段旁白起点偏移不超过0.29帧。未连续观看或听审，尚未生成正式视频ZIP。

终检修订第一批已放大跨端回答、账单金额及来源、架构与片尾关键文字，
实际PNG字高33–38px；同类检查与整片复核仍待完成。操作习惯和安全策略已通过
合成公共接口样例取得真实提取结果及版本历史，原输出风格版本三保持不变；
新增结果尚未编入视频，不能因此宣布三类展示或最终交付完成。
...
第24版已整合三类偏好结果、来源正文放大和动作音效。限定范围独立复核
确认对应画面/原始结果及装配缺项闭合；不代表全片听审或正式交付通过。
...
完整解码、源快照摘要、29段旁白起点、283张抽帧OCR、5帧工作台逐像素对照、ZIP单成员/CRC/摘要、冻结目录和其余作品未变核对均已完成。OCR无关键词命中，-40dBFS阈值未检测到持续1.5秒以上静音；实际主观听感未人工连续试听。独立实帧复核及技术报告见 `submission/video-production/review/promo-revision-03/`。旧版MP4、ZIP及中间媒体保存在制作工程，Git保留每轮变化。
