...
[正式截图素材](../submission/00-原始记录/截图素材索引.md)当前待重新采集，
失效图片及其画廊、清单条目已移除，不保留旧版展示归档。目录与章节继续保留，
清单为空不代表素材交付完成；须在唯一前端及对应安装包实际验证后补齐图片、
来源提交、SHA-256 和使用边界，不能沿用迁移前截图作为下面状态矩阵的证据。
...
仓库根目录 [`submission/`](../submission/) 是唯一且直接可见的交付实物区，由团队
手工维护，不混放生成脚本、测试或中间文件。`docs/delivery/` 保存可审查的 Markdown
正文；交付 PDF 与对应 Markdown 使用相同文件名并放在同一目录。D-01～D-05、D-02
附件 A-01～A-05、报名表、安装包与证据由负责人按最终 release commit 逐项复核。
...
最终交付复核逐项检查 PDF/DOCX/PPTX/ZIP/视频的真实格式、`.deb` ar 头、摘要文件中的
文件名与实物 SHA-256，以及固定 Ed25519 公钥签名。A-02 原始证据覆盖双 SDK、Agent
生命周期、性能、三设备、安装升级、数据集和供应链，并与最终包及 release commit 一致。
