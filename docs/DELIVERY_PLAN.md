...
| Module E 随包安装 | 尚未实现 | 阻断 |
| V11 双 SDK 严格画像 | 最终全链路证据未完成，H-02 未通过 | 阻断 |
...
不得只在多个源码文件手工重复版本号；当前 `0.1.7` 多源同步机制应收敛为发布预检
可证明的一致性，而不是依赖人工记忆。
...
4. `dpkg` 阶段不可强杀；成功后执行服务/Agent/provider 健康检查。
5. 图形界面完整呈现下载、校验、授权、安装、重启、取消、失败和恢复状态。
6. 更新 PIXIU 自有包，不静默升级上游 Agent 或系统 SDK；兼容性不满足时明确阻断。

当前 `UpgradeController` 已实现版本比较、按架构选择 `.deb`、流式 SHA-256、重定向
白名单、`pkexec` + root-only 二次校验与错误分类。最终仍需补：独立签名、Module E
兼容矩阵、失败回滚、安装后健康检查/受控重启，以及最终 V11 图形升级取证。
