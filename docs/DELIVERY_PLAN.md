...
用户提供新版赛方图片并确认平台登记信息后，当前目录命名改为“作品编号-作品名称”，取代上文历史冻结记录中的学校与姓名命名。§0 保留原始历史记录；本次明确授权按最新图片执行，四项内容与匿名要求继续适用。
...
机器读取值维护在 `docs/submission-identity.json`。内层只有项目报告.pptx、技术方案.docx、演示视频.zip；源代码目录与内层并列，保存 PIXIU源代码.tar.gz、x64安装包及SHA-256校验文件。用户最新编辑并完成政策、研究说明及互连示意修订的36页报告为当前正式提交源，制作模板仅作历史资产。

用户进一步明确要求匿名指导文档禁止个人仓库链接，并将x64安装包加入源代码目录供直接安装；其他平台使用源码编译安装。本次以此最新要求执行。
