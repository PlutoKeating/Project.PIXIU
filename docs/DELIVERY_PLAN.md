...
| 功能演示视频 | 1 份，5-10 分钟，`演示视频.zip`，大小不超过 200M | `delivery/PRESENTATION_AND_VIDEO.md`、`submission/video-production/` | 第十七版中文初剪已生成，9 分 21 秒、约 58.0 MB；同步检查点、历史量化表、高清偏好堆叠、非标题搜索、洞察来源链、跨端遗忘检查点、设备信任及共享边界控制已入片，整片解码通过；洞察日期字号与来源时序通过有限复核，仍有功能/模板细节/视听余项，正式 ZIP 待完成 |
...
已保留原记录并更正；原生检索与来源正文画面已补录，尚待接入影片，不扩展为Wayland原生采集验收。
