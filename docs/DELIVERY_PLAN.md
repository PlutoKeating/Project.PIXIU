...
[正式截图素材](../submission/00-原始记录/截图素材索引.md)现有 38 张 0.1.9 V11 实拍，
...
2026-09-08 首次检查发现材料过时、缺少操作截图及身份信息问题，历史结果见
[实物审查记录](delivery/DELIVERY_AUDIT_2026-09-08.md)。随后已修订中文正文，
将 38 张真实操作截图用于手册、部署说明、应用案例和 PPT，并逐页复核重新导出的
全部 PDF、Word 和 PPT。当前结果见[截图与文档复核记录](delivery/SCREENSHOT_REVIEW_2026-09-08.md)。

V11 已重装并运行 0.1.9-1，运行主程序摘要与包内文件一致，三个用户服务运行正常。
完整场景截图、停止任务后的状态显示修复、同版量化评测、正式源码归档和最终目录
仍待完成。当前正式源码包仍为 0.1.7；0.1.9 候选归档已生成并完成逐文件摘要检查，
尚需检查独立构建、文档引用及匿名内容。
