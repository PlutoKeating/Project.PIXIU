...
三变体原始结果必须由 `agent-memory-ablation.py` 基于仓库固定的 30 项任务集产生。每项
均跨教学/召回两个独立会话；无记忆组显式关闭所有 Runtime 记忆面，单机组关闭同步，
分布式组在同一已验证三设备拓扑的不同节点间教学和召回。工具只保留逐任务成功布尔、
轮数、工具名和摘要并复算结果。当前 6 项采集器契约测试通过，真实 V11 结果尚缺。
