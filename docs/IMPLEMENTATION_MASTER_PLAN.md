...
| W1.4 | B/C 组合根 | 实现 Kylin VectorStore 适配并注入写入/检索/遗忘链；保留原始 float 向量到 SDK | 当前源码已补生产 `LoadDBFile`、进程级复用与 `Disconnect`；792 项组合回归通过，V11 新包待 W1.6 |
...
并以当前 792 项测试回归。提交 `6f6002e` 的 strict revision 8 已重建并通过写入、查询、
