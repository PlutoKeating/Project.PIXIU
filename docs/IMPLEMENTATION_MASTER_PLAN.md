...
W5 已落地 `agent-lifecycle-evidence.py` 及受控场景：两阶段分别位于宿主/Runtime 重启
前后，绑定通过的 strict V11 双 SDK 原生证据与同一候选包；机器门要求三轮会话、
Shell、联网搜索、PIXIU 记忆读写、现场一次性审批、工具结果跨重启持久化和新会话
召回全部通过。采集器的 8 项契约测试已通过；最终 V11 模型运行尚未执行，W5 仍为
进行中，不能凭工具存在宣称闭环。受控场景已根据首轮目标实测收紧为一次性临时文件
清理审批与联网后长期沉淀，并新增发行场景自身的校验，现为 8 项契约测试。

宿主 UI 已按 ADR-0004 进入受控补丁链：移除四个主页面中的局部硬编码颜色，建立
双主题语义令牌，重做品牌栏、会话侧栏、云端模型入口、消息/空状态和输入区，并增加
补丁形状与关键配色对比度测试。该切片只有在同版 V11 重建后完成双主题、三档缩放、
