...
W6.6 的最终汇总门已由 `final-performance-evidence.py` 实现：强制三个同任务集变体、
每组不少于 30 例，并将四项官方指标的逐样本报告绑定到 strict V11/Agent/冻结数据集/
...
| D-07 | 效果/测试报告 | 🟡 A4 PDF、三设备总 JSON 与七类原始证据归档门已接线；后者强制双 SDK/Agent/性能/三设备/安装升级/数据集/供应链同 commit 和候选包摘要并扫描敏感内容；仍待真实输入后生成 |
