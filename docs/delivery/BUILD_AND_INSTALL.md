...
本章给出直接安装与源码编译两条完整路径。终端框中的$和>是提示符，不属于命令，输入时不要复制；反斜杠表示命令续行。希望体验产品时使用路径A；希望审核实现并重建安装包时使用路径B。两条路径最终都得到同一个PIXIU桌面入口。
...
作品提交包的“源代码”目录已经提供x64安装包与校验文件，无需访问代码托管网站或另行下载安装包：
...
校验应显示OK，安装应正常结束，随后出现PIXIU主窗口。需要重建时，可按路径B使用提交的源码生成同名DEB。SHA-256用于确认下载完整性，软件内更新另有Ed25519签名校验。
...
随附DEB只适用于银河麒麟V11 x64，不能直接用于ARM64、其他CPU架构或非兼容发行版。其他平台应解压PIXIU源代码.tar.gz，在目标平台编译并安装，不通过外部项目主页获取平台包。

对于采用Python3.12的Debian系x64环境，可使用源码中的generic-ubuntu画像作为兼容构建起点：依赖预置时将画像名改为generic-ubuntu，准备供应链前设置PIXIU_KYSDK=OFF，生成安装包时设置PIXIU_PROFILE=generic-ubuntu及PIXIU_INSTALL_STRICT=0。其他步骤沿用路径B；系统软件包名称须与目标发行版匹配。此模式使用可移植实现，不代表麒麟原生SDK能力或评测结果。

ARM64等其他架构目前没有已验证的整包构建锁，不能仅改一个架构变量就宣称适配完成。应先在目标机编译宿主，按目标CPU与Python ABI重建Runtime依赖及其哈希锁，再增加对应build/release/profiles画像，并调整供应链脚本中的amd64校验与依赖选择。随后重新执行供应链核验、打包和安装验证。当前提交提供这些构建源码，不宣称其他架构已通过安装验收。
