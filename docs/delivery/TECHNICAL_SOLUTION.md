...
会话列表关闭横向滚动并对长标题右侧省略；消息采用独立行容器管理 Qt 所有权，避免
刷新或折叠侧栏后产生重复气泡和残影。
推理选择面只保留 DeepSeek、Anthropic、OpenAI 官方直连云端提供商，默认补入并优先
选择 DeepSeek 官方 `deepseek-chat`；本地服务和 OpenRouter 中转均由宿主过滤。密钥
归 Runtime 安全认证存储所有，不属于模型清单、源码或交付证据。
...
H-02/H-03 技术门已由 `d3b6bae` strict 同包原生证据通过；`CONVERSATION`/provenance/完成态
