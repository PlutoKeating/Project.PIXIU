...
`d3b6bae` 候选进一步把可重建 Agent 宿主、锁定 Runtime wheelhouse、Provider 与双 SDK
...
| openKylin Agent + Module E | `d3b6bae` 同包宿主/Runtime/Provider、离线供应链、Gateway 与会话 API 通过；Mock 工程链通过，官方云端模型 run 未完成 | 未完成 | 已有可重建/离线/工程链证据；待官方云端完整 run |
| Embedding SDK | `d3b6bae` strict 同包 native compliant，产品链使用 runtime 1.3.0/gte-base 768 维向量 | 技术门通过 | 最终冻结 commit 的正式归档 |
| Vector Engine SDK | `d3b6bae` strict 同包 native compliant，direct SDK 和产品写入/检索/遗忘/隐藏通过 | 技术门通过 | 最终冻结 commit 的正式归档 |
