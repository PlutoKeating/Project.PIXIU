...
- 旧 KylinAgent 截图暴露主题冲突和内部路径后已判定失效；ADR-0004 下游 UI 补丁、
  ...
  截图矩阵仍待绑定新候选执行。
- 宿主模型列表已增加静态门禁：DeepSeek 官方 `deepseek-chat` 优先，产品只展示
...
- 真机重打包复核发现宿主供应链记录原先只绑定固定上游 commit，未绑定 PIXIU 下游
  适配输入，因而新 UI 提交可错误复用旧宿主实物。当前门禁已要求宿主记录同时匹配
  release commit，以及构建脚本、两份补丁和兼容层源码的逐文件 SHA-256；任一变化都
  必须重新构建、记录和审计后方可进入 strict 包。
- 深色主题真实桌面回归已确认消息收敛到 920px 居中阅读列、助手正文正常行宽、用户
