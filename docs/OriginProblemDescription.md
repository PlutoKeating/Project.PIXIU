...
> [!IMPORTANT]
> 本文保留赛题文字方案及附录 A 原始内容。2026.05 的赛方宣讲材料
> [`完整赛题要求.pptx`](完整赛题要求.pptx) 是必须联合执行的权威依据，新增了
> 银河麒麟桌面操作系统 V11、系统向量数据库 SDK、系统 Embedding 接口三项硬门槛，
> 并明确 OS Agent 应具备多轮会话、规划、工具调用和记忆能力。可执行的合并口径见
> [`AcceptanceTestSpecification.md`](AcceptanceTestSpecification.md)。

## 权威口径补充（来自 2026.05 PPT）

### 零分风险与指定技术栈

1. 软件必须部署在**银河麒麟桌面操作系统 V11**；PPT 明示不满足计 0 分。
2. 数据库必须使用系统向量数据库 SDK（`kylin-ai-vector-engine`，本仓库对应官方客户端 submodule `third_party/libkysdk-vector-engine-client`）。
3. 文本向量表示必须使用系统 Embedding 接口（`kylin-coreai-embedding`，本仓库对应同名官方 submodule）。

SQLite 可以继续保存元数据、关系和审计，但不能替代指定向量数据库承担最终生产向量读写与检索。portable/hash/stub 仅用于 Debian 开发回归，不能作为上述硬门槛的通过证据。

### OS Agent 和记忆定义

PPT 将 OS Agent 定义为运行于银河麒麟 V11 的系统级智能助手：能理解复杂指令、自主规划、调用系统/外部工具并执行任务、适应用户习惯，而不只是聊天。记忆来源包括多轮对话、工具结果、用户行为和手动配置；短期记忆服务当前上下文，中期记忆保存项目/会话状态和摘要，长期记忆跨会话持久化并由检索注入当前上下文。

本赛题要求优化的是完整 OS Agent 上的记忆能力，而不是要求参赛队从零重写所有通用 Agent 能力。PIXIU 决定复用 `third_party/kylin-agent` 与 `third_party/kylin-agent-runtime`，通过记忆提供者接口接入；详细依据见 [`OS_AGENT_INTEGRATION_ASSESSMENT.md`](OS_AGENT_INTEGRATION_ASSESSMENT.md)。
...
**2026.05 PPT 对初次作品提交的明确补充：**

1. 项目报告 1 份（PPT）；
2. 技术方案及测试结果 1 份（Word/PDF）；
3. 源代码及代码规范 1 份；
4. 部署文档 1 份；
5. 演示视频 1 份，格式为 `.avi`、`.mp4` 或 `.wmv`，时长不超过 7 分钟。

合并执行时还应保留文字方案要求的用户手册、效果验证报告、记忆流转说明、至少
1 个真实应用案例和银河麒麟适配报告，详见完整验收清单 D-01～D-10。
...
> PPT 饼图显示 32%/32%/26%/11%，是将前四项 30/30/25/10（合计 95）自动
> 归一化后的显示，且漏画文档规范性 5 分；不得据此把原始评分改成 32/32/26/11。
> 初评仍按上表 30+30+25+10+5=100 分执行。
...
> **时间冲突提示**：下列日期来自文字方案；2026.05 PPT 则写“10 月初作品提交、
> 10 月作品评审、11～12 月总决赛”。仓库不再把任一日期视为无条件确定值，团队
> 必须以赛方最新正式通知确认并保存证据。
