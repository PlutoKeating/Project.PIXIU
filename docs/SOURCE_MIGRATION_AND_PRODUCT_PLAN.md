...
| P10 | GitHub #2 | 图片账单导入、识别确认、保存、回查不完整 | V11 使用实际 OCR；提供图片选择、结果确认、保存及来源查看；非原生环境明确能力差异。该项不是文字账单分镜 s07 的前置条件 |
...
| 阶段 | 内容 | 退出条件 | 状态 |
|---|---|---|---|
| M0 | 记录基线与保存计划 | 当前提交、工作区、安装路径、测试入口明确 | 进行中 |
| M1 | Agent 与渲染资源迁移 | 插件测试、导入和资源检查通过 | 待开始 |
| M2 | build 产品实现迁出 | 补丁可应用、启动/初始化/升级/迁移测试通过 | 待开始 |
| M3 | 正式前端与测试整理 | 管理库与回归编译、界面测试通过 | 待开始 |
| M4 | 验收工具、交付与文档接线 | 无运行时旧路径引用，导出清单完整 | 待开始 |
| M5 | 干净源码构建 | Git 工作区与无 .git 源码均有可用构建入口 | 待开始 |
| F1 | P01/P04 范围贯通 | 私人采集及授权共享可被助手使用 | 待开始 |
| F2 | P02/P03 偏好与账单 | 新会话偏好生效、金额按问题正确计算 | 待开始 |
| F3 | P05–P09 其余宣传链路 | 用户能完成处理，沿用现有界面 | 待开始 |
| F4 | P10 图片账单 | 识别确认保存查询来源；原生验证单列 | 待开始 |
