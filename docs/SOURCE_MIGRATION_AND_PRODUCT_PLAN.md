...
批准日期：2026-09-10。用户已完整批准本计划对应的只读审查方案，并授权跨模块实施、本地验证与提交。执行优先级：先完成干净的源码目录迁移，再补齐普通用户使用宣传场景时的明显阻碍。优先快速交付，不扩展为内部技术加固、泛化框架或穷举异常工程。后续用户已授权发布，0.1.12 已上线。
...
| P10 | GitHub #2 | 图片账单导入、识别确认、保存、回查不完整 | 按用户最新要求：使用配置的多模态模型理解图片并生成结构化知识草稿；提供图片选择、结果确认、保存及来源查看；OCR 仅用于脱敏标注。该项不是文字账单分镜 s07 的前置条件 |
| P11 | 交付 | 已发布包落后于当前代码修复 | 0.1.12 正式包已发布，安装、升级和三端共享验证通过 |
...
| M5 | 干净源码构建 | Git 工作区与无 .git 源码均有可用构建入口 | 宿主完成：4634 文件导出校验、无 Git 宿主编译通过；Runtime 两次独立构建一致，锁校验及离线安装通过 |
...
| M6 | P11 完整候选复核 | 新装、升级、会话、来源、同步、采集、遗忘、桌面能力结果记录 | 0.1.12 已发布；新装、升级恢复、三端共享及并发更正通过，详见发布记录 |
...
无 Git 源码模式：构建脚本用交付清单核对固定上游快照和版本/构建时间，完成相同补丁与构建；不要求评委取得原仓库或补造 .git。保留文件模式与必要符号链接。正式包绑定标签提交和签名清单。
...
- 产品修订：用户明确图片知识提取应使用多模态模型，撤销 OCR → 正则草稿作为正式图片账单入口。保留已实现的确认编辑/原图追溯；OCR 仅用于脱敏标注，不宣称能够独立保证敏感信息检测。

## 9. 2026-09-10 用户体验修订（覆盖此前图片模型选择方案）

统一原则：正常使用只围绕目标、计划审批和进度/结果展开。技术能力自动判断，常规执行不要求用户重复选择。图片与扫描 PDF 从聊天附件、授权目录监视等入口进入同一读取链路；使用当前聊天模型，不另设图片模型选择。不支持多模态时停止对应资料读取，在相关设置中说明不可用；OCR 仅用于脱敏标注，不承担知识提取。

| 当前实际入口/阻碍 | 调整 | 保留的用户介入 |
|---|---|---|
| 图片导入要求再次选择模型 | 删除选择框，跟随当前模型能力 | 无 |
| 图片识别后必须逐条填表才能使用 | 附件/授权监控由 Agent 自动理解处理，人工编辑作为需要修正时的工具 | 不确定内容影响任务时仅询问必要事实 |
| 聊天宿主附件声明存在，正式简化界面没有完整接线 | 接通文件附加与直接提交；与目录复用读取能力 | 用户主动附加文件即可 |
| 目录混合文档需要自动理解与能力判断 | 改为当前模型理解；无能力时禁用并显示设置状态 | 首次目录访问授权 |
| 采集设置、服务/设备/阶段页面依赖手动刷新 | 页面自动加载，相关事件后自动更新，保留非阻塞手动重试 | 无常规刷新要求 |
| 保留阶段记忆按钮再次确认同一动作 | 去除重复确认；已批准任务由 Agent 自主留存必要记忆 | 不重复询问 |
| user:/shared: 范围及内部状态直接暴露 | 正常路径使用私人默认与已授权共享设置；高级管理保留内部标识 | 首次共享授权，不让模型擅自公开私人资料 |
| 冲突必须用户挑版本 | 模型依据已知来源/明确用户更正处理，无法判断的实际事实才询问 | 实质性歧义 |
| 遗忘预览与确认跨页重复输入 | 沿用意图交接，审批围绕影响，不让用户搬运参数 | 未授权不可逆删除仍须明确授权 |
| 配对、解除信任、安装提权 | 复用系统必要授权，不冒充计划审批能替代 OS 权限 | 首次信任/系统提权 |
| 工作完成只显示“请刷新核对” | 主动报告已经完成的工作和需要注意的业务结果 | 无 |

本修订不是仅改提示词：入口、能力判断、读取调用链和常规页面自动更新需要实际接线并验证。没有真实图片模型时必须记录限制，不能用文字或 OCR 成功代替多模态验收。

## 10. Dreaming 扩展

用户新增 Office/统一 MCP/受控后台 Agent 整理要求，具体接口、覆盖与执行次序见 [DREAMING_AND_DOCUMENT_HARNESS.md](DREAMING_AND_DOCUMENT_HARNESS.md)。已接入正式包；具体测试结果见发布记录。
