...
自动化复算 4.5:1 对比度；长会话标题采用右侧省略且关闭横向滚动。宽屏对话改为最大
920px 的居中单列阅读流，用户消息在列内右对齐，PIXIU 回复以作者标识和无重边框正文
连续排列；输入组合区限制为 960px，最新消息在布局完成后再次滚动到可见区域。
宿主供应链记录绑定 release commit 与全部下游适配输入摘要，旧二进制不能进入新候选。
最终状态仍须绑定新候选包完成
