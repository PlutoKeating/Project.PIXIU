...
| H-02 | 使用系统向量数据库 SDK | **未通过**：direct SDK 生命周期已通过；真机发现生产 DI 漏调 `LoadDBFile`，当前源码已修复并增加进程级连接释放，待重建候选包复验产品链 | SDK 建库、写入、查询日志与端到端测试 |
| H-03 | 使用系统 Embedding 接口 | **未通过**：修复上游组件契约后，官方 demo 与 PIXIU 绑定均已返回真实 768 维向量；该修复尚未成为最终可安装依赖，且用户会话产品链待闭环 | `runtime=kylin` 报告、调用日志与故障即失败验证 |
...
当前源码已补数据库路径、严格启动装载、进程级复用与关闭，待新候选真机复验。
