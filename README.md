...
| H-02 | 使用系统向量数据库 SDK | **未通过**：V11 direct SDK 已完成独立数据库建库/写查删/清理；系统服务与桌面用户 runtime 的 UID/socket 边界尚未解决，产品生产链证据未生成 | SDK 建库、写入、查询日志与端到端测试 |
| H-03 | 使用系统 Embedding 接口 | **未通过**：绑定和原生编译已完成，但同一用户会话边界问题阻断严格后端调用，最终运行证据待补 | `runtime=kylin` 报告、调用日志与故障即失败验证 |
