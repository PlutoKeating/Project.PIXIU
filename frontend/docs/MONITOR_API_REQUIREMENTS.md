...
目录浏览复用宿主现有对话框，只追加待保存草稿，不发送 PUT 或开启采集；取消及重复
选择保留草稿。浏览器返回相对路径、含首尾空白或换行的路径时拒绝添加，避免路径被
当前逐行编辑和后端归一化改变；配置读取前、请求在途或目录选择期间禁止发起保存。
...
现有列表展示 UTC 时间、来源、状态、摘要及证据/知识 ID。时间按整秒解析，缺失、
非法或超出可显示年份范围时明确提示，不替换为当前时间或 1970 年；缺失标识不推测补齐。
可选择记录后通过右键或 Ctrl+C 主动复制，刷新不会自动写剪贴板；公开前须核对私人信息。
这些关联标识不代表已验证对应记录仍存在；完整取证与原生排版仍按统一计划验收。
