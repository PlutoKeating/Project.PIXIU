...
> **状态**：✅ 已实现（2026-08-26）；契约转正于 `docs/API.md`，本文档保留为
> 前端消费方视角的契约快照。
...
- 本文保留最初消费方需求；相关端点、transport 与监控中心已经接通，
  不是待评审接口。当前权威契约为 `docs/API.md`。
- 活动记录使用秒时间戳；WS 不提供历史补发，断线后通过 `/monitor/log`
  分页补读。四个开关不代表四种系统采集器都已完成：目录与行为采集已实现，
  剪贴板/截图的开关及配置不能充当原生捕获验收证据。
