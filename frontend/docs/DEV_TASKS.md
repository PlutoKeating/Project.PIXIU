...
主窗口再经过检查，导出断言保证接线。Agent 草稿/运行任务、直接 quit 的升级重启、
托盘和真实桌面退出矩阵仍未完成，不能套用旧 PixiuApp 的验收结果。
...
与有效摘要显示。洞察依赖后端固定个人域和推荐抑制规则，简报仅聚合采集日志；
...
管理测试现为五组；HTTP 详情测试补齐路径、范围编码与嵌套正文读取。
