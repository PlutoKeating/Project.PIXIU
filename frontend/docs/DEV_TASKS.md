...
`MemoryWorkspace` 已独立呈现 schema v13 的 `capture_source`，不将其插入正文或
高级 raw JSON。只读区域有高度上限，支持长路径滚动复制，换行文件名按转义显示；
缺失与非法来源分别使用紧凑可选取标签提示，隐藏并清空来源文本框；有效来源才显示
独立文本框。来源列表高度上限为 72 像素，正文最低 100 像素且优先分配剩余空间。
不从文件名推断路径，也不宣称原文件仍可打开。来源/范围
切换及数据失效会清除旧元数据。管理 12 组回归及宿主导出检查通过，新增边界用例
覆盖文本/OCR、长路径、空值、非法类型/方法/时间/路径、共享域及失效后迟到响应。
沿用既有管理库 CMake 和导出文件，无新增依赖/构建目标；V11 同版展示尚待验证。
...
MemoryScopeControl 已给检索、录入、偏好和遗忘增加“其他范围”确认入口，复用
既有选择器数据与信号。测试覆盖格式拒绝、超长不截短、默认回车取消、重复去重、
请求禁用及确认期间状态变化后的再次拒绝；跨入口请求保留明确范围，选择不发
写入/遗忘请求。控件复用 Qt Widgets，管理构建及宿主导出同步更新。核心模型已
修复点号范围拒绝及尾随换行接受问题；各入口长度上限仍需统一，不以当前 UI
回归证明所有历史范围均可完成写入或真实安装验收。

唯一宿主新增可选 `HostTray`，显示动作恢复同一主窗口并保留最大化状态，退出动作
经过主窗口关闭保护；无系统托盘时保留普通窗口入口，关闭最后窗口仍退出。
组件测试覆盖显示、最小化恢复及忙碌时拒绝退出；麒麟托盘实显与退出矩阵待验证。
补入正式宿主的屏外恢复：唤起时保证普通窗口有可达标题区域，否则移回主屏；
不变更尺寸，不重置正常位置，保留最大化/全屏。回归先复现只显示仍停在屏外，
再覆盖正/负屏外、边缘残留、重复唤起和窗口状态。沿用现有 Qt Gui/Widgets 与
HostTray 导出配置，无新增依赖；真实多屏/Wayland 验证及旧几何测试清退仍待完成。
Ctrl+Alt+P 复用现有快捷键适配，ON 使用画像已列出的 kysdk-shortcut；OFF/失败仅
应用内生效。新增 Qt 重复注册/释放回归与测试专用 SDK 替身归属回归，替身不进入
导出产品、不作为原生结果。已新增注册前删除旧 PIXIU 专用绑定的升级迁移，回归
覆盖清理顺序及失败继续注册，不触及其他名称。V11 原生 `ca7e6f8` 托盘恢复通过，
全局按键失败；实机清理旧绑定后仍失败，启动官方 kdkshortcut 会话服务后恢复成功。
已补入当前会话的服务就绪与失败降级处理：仅缺失时启动官方程序，等待时使用
本地快捷键，D-Bus 归属改变更新状态及托盘说明，不终止共享服务；组件测试不启动
真实桌面服务。原生画像显式依赖 Qt DBus 运行库；`96d70e9` 已通过实际首次启动
自动启动 SDK 服务与按键恢复验证。另补 SDK 配置缺失但 compositor 自有注册残留
的定向恢复：仅创建/更新都报告已存在时尝试，一轮后仍冲突则降级，不影响其他名称。
回归覆盖恢复成功、仍冲突、清理失败及其他 SDK 错误；`eb5875f` 原生作业
34175941456 通过，实际复现残留后由软件自动恢复并启动官方服务，真实按键恢复
同一窗口，托盘全局状态一致。真实跨应用冲突与完整通知矩阵仍待验证。
完整 OFF 宿主编译与动态依赖检查通过；导出回归核对真实适配源码、官方链接声明，
并拒绝测试用 SDK 头进入导出树。该隔离检查不能代替 ON 桌面服务验证。
严重冲突提醒已接入唯一宿主和同一托盘，复用 NotifyService；ON 使用已有通知 SDK，
OFF 降级。测试验证固定文案信号无载荷、限流、普通与未知级别不弹，不触发确认。
偏好列表变化提醒已迁入同一宿主，scope/id 版本基线隔离首次加载、范围切换、重复
及回退；损坏或重复记录不推断变化。信号只带数量，固定通知文案不暴露偏好内容，
不把截断列表变化称为实时学习事件。回归覆盖上述边界且不写入；原生通知展示
仍待完成。旧相关主题的文件名/标题字串推断、缓存、每日计数、接线及两项专用
测试已删除，不移植无证据的语义结论。正式事件回归覆盖采集四状态的页面提示与
无正文刷新信号，不转发文件名/业务标题，不发命令或误触发严重冲突通知；可见
采集日志自动刷新及应用内洞察读取继续保留。旧生命周期的偏好提醒函数、缓存和
重复测试已清理，新的
范围隔离、回退和异常响应回归保留在正式管理模块。旧简报全文系统通知、接线及
专用测试数据已删除，正式 DeliveryPage 保留日期与正文回归。`eb5875f` 原生验证
今日/历史空简报与接口一致，改日期清空旧正文；随后真实目录采集合成文本，接口
与界面均显示“当日新增 1 条记忆（目录 1）”。该路径不覆盖非空洞察或所有来源统计。
另在 `66c3cbd` 的 V11 自动安装后正常重启，核对进程/安装文件一致，完成正式
洞察页非空候选、标题检索和来源正文的真实操作，输入沿用目录采集的合成文件。
质量分按两位小数展示，检索限定 `user:local`。此结果不覆盖全部候选和异常状态，
不解决文件来源元数据缺失或偶发导航隐藏，不作为最终 Release/截图库验收。
其他旧测试需继续核对后移除。
旧 `DeliveryController` 及启动/窗口显示时的洞察自动请求接线已清退；两个旧
生命周期测试目标不再编译它，专用导航用例及假数据已移除。正式 DeliveryPage
回归确认不隐式发起启动请求、候选标题/摘要/质量分真实显示、标题检索与错误/空态
有区别；历史日期和迟到响应回归继续保留。其他旧聊天控件仍待逐项清退。
旧聊天动态洞察卡的孤立渲染方法、卡片类、成员及两项专用测试也已删除；正式
DeliveryPage 的回归保留。现有 CMake 测试目标继续编译其余旧壳，不新增目标或
依赖，产品导出不包含被清理的旧聊天控件。
`HostCloseGuard` 统一检查所属对话框、在途管理请求及采集/同步
配置编辑；默认取消退出，放弃不发写请求。组件回归覆盖取消、明确放弃、请求失败
及退出确认的明确动作文字；使用真实 Return/Esc/按钮点击验证默认保留与主动放弃，
不再只用 QMessageBox 返回码模拟交互。原生 `eb5875f` 已验证未发送合成草稿的
窗口关闭保护、最小化后 `--quit` 恢复和 Esc 保留、明确放弃后菜单重开且原会话
列表仍在；没有发送草稿，采集配置保持不变。新按钮文字原生复验及完整请求/升级
矩阵仍待完成。`d39661f` 原生复验发现 UKUI 会覆盖标准按钮的自定义文字，现改为
具名自定义动作与 clickedButton 判定；同环境最小测试显示文字保留。回归另拒绝
没有放弃动作的 Yes 返回码，并新增原生 UKUI/Wayland 流水线门；新宿主验收尚未
完成。原生测试曾因 UKUI 重挂按钮而空指针崩溃，现按当前活动模态窗口查找动作，
并排除前次对话框暂留的隐藏按钮。相同原生环境完整退出控件用例已通过，未放宽
默认保留与 clickedButton 判断；完整流水线和安装后实显仍须复验。其余组件回归继续覆盖请求失败
...
正式设置页新增三个只读说明入口：关于、数据与联网、许可证与第三方组件，复用
`InfoDialog`，不另起前端或配置存储。`ProductInformation` 统一正文，撤销旧声明中
仅存本机、永不上传及任意清除的过度承诺，明确共享域、云模型、采集和遗忘边界。
旧回归入口同步使用正确正文；旧翻译源中的三段错误内容移除并重新生成 `.qm`。
回归覆盖产品版本、数据边界、随包声明路径、只读属性、重复打开复用、关闭不退出
宿主及宿主可见对话框保护；导出检查核对共用源文件字节一致。原生画面和完整多语言
仍需验证，不据组件测试勾选全部设置迁移。

嵌入隐私页新增加载前禁止保存、配置字段形状检查、绝对路径与目录去重、失败编辑
保留、未保存提示及日志分页。测试验证不可用来源的原配置值不会因保存其他选项而
被覆盖。采集事件及重连合并刷新可见页日志首页，在途延后、历史页不跳转；读取日志
期间仍可编辑配置，不自动重读或保存配置，结果保留未保存提示。回归覆盖上述读写
边界与宿主接线。真实采集运行与桌面能力降级仍须单独验证，不能仅依据配置响应标通过。
...
事件失效会清空洞察与简报但保留历史日期；在途响应结束后丢弃，旧候选不可继续
用于标题检索。回归覆盖历史闰日保留、迟到响应拒绝和手动重新读取，无自动写入。
...
会话/后端变化还会清空旧会话来源，组件测试覆盖在途证据迟到后正文仍为空。
`fa7dfd5` 原生包已从开始菜单启动并实际完成会话来源→当前证据读取；运行哈希与
安装文件一致。上下文清理属于后续 `5940f08`，尚未原生验证。来源模式空白检索区、
按钮与品牌的位置、选中态及 `body.content` 正文优先仍待修正，不作为最终截图验收。
后续实现已隐藏来源模式空白回答区，并在普通检索恢复；`body.content` 正文优先及
额外字段保留已加入组件测试，按钮移到品牌之后。来源选中态样式已补齐；模拟原生
冲突绘制的浅/深/浅、有焦点/失焦渲染测试先失败后通过。上述修正尚待原生复验。
`98259cb` 的浅色原生界面已验证按钮、正文布局及选中态；会话切换实测暴露宿主
未发通知，已补 `loadSession` 的 ID 变化信号及导出检查。`53dc32d` 原生包已复验：
读取来源及正文后切换另一既有会话，返回记忆页时旧列表、正文清空；深色来源选中态
有焦点和失焦均可读，恢复浅色仍可读。该结果不代表所有错误态和完整链路验收通过。
浅色未勾选复选框标识不可见已在原生界面复现；勾选后原始数据切换正常。`0019`
为未勾选标识补充边框、背景、焦点与禁用样式，导出渲染回归先失败后通过，并检查
键盘切换、可区分勾选态和禁用态不变。该修正尚待新原生包复验，不新增资源或依赖。
沿用 Qt Network，
...
共用 `contract_fixtures` 已接入管理模块测试，沿用同一 HTTP 实现，不额外编译旧
窗口。遗忘替身改为缺少凭证/错域/复用拒绝、错域后重新预览，并返回知识目标 ID，
不再将级联证据 ID 混入 forgotten_ids。此项为 TCP 替身传输验证，不是实际删除验收。
检索页已接入事件失效通知：清除旧结果与证据，保留输入与范围，迟到查询及证据
不得重新填充正文。测试核对在途串行门控和不自动写入；原生事件路径尚待验证。

嵌入设备页已接线状态/节点读取、设备广播、启用/暂停设置和单节点本地信任解除。
设备事件与重连已接入合并只读刷新，隐藏、有未保存开关或在途操作时延后；用户
恢复原值或保存结束后再读。所选节点仍存在则恢复，移除则清空选择并禁用解绑。
撤销信任确认框纳入忙碌门控，嵌套事件循环中不会自动刷新确认对象。测试覆盖事件
合并、未保存编辑保留、在途等待、选择恢复/移除及默认取消不写入；沿用既有导出
清单和 Qt 依赖，原生事件与双设备交互仍待验证。
`ee5da25` 原生复核已观察到启动事件栏、显示设备页和偏好页自动读取；设备开关
草稿切页保留且后端 enabled=false，撤回草稿后两个开关均关闭。首击曾切到终端，
任务栏恢复后导航成功，未确认原因。该进程检查期间新包替换了磁盘文件，不将旧
进程截图计入新包验收；真实设备事件、断线恢复及双设备状态仍需验证。
...
宿主 `BackendEventStatus` 已接入只读事件状态提示，复用 WebSocket 客户端，不复用
旧事件路由的遗忘授权接线。广播只合并受影响页面名称；重连提示重新读取，不自动
覆盖表单，不显示事件命令/正文或发送写请求。管理库链接 Qt WebSockets，发布画像
已有依赖；新增组件、客户端及 `0023` 均已加入导出和供应链清单。审计页通过事件
标记、合并定时器和页面显示状态自动刷新；在途历史或提取结束后才读列表，按 ID
恢复选择并重新读取历史，记录消失则不保留旧详情。提取失败后只读核对，不重放写
操作。测试覆盖隐藏、事件合并、在途等待、选择恢复/消失与提取不重复；不新增依赖。
其他页面自动刷新、严重程度通知及原生完整链路仍未验收；Qt 5.15 消息/帧上限与
旧 Qt 解析上限分开看待。

嵌入 `MemoryAudit` 已接线偏好列表、历史、提取与冲突审计：历史由列表选择触发；
提取仅使用现有证据 ID；请求中禁用重复操作，错误不会显示成“暂无记录”。历史响应
核对所选 ID，失败可重新选择记录；冲突仅展示实际后端仲裁结果。列表补充后端关联
记忆标题及中文严重程度/结果说明，同时保留原始枚举；未知值原样保留，缺失值不
伪装成功。详情保留新旧双方，明确 MANUAL 尚待人工确认、界面不提供裁决或回滚。
组件回归覆盖标题、已知/未知/缺失结果及内容保留；仍待真实非空记录的原生复验。
沿用 `MemoryAudit` 的既有导出清单、公共接口与 Qt 依赖，不新增打包输入。
`ef7802f` 原生界面已读取已有偏好及两个历史版本、冲突双方；尚不包含上述展示补充。
`ForgetPage` 预览关联数量缺失或非法时显示“未知”，不再默认显示零；合法零值保留。
回归覆盖缺失、负数、非整数、错误类型与合法计数，不改变确认、取消和一次性凭证
语义；无新增依赖或打包输入，仍需完整原生遗忘路径验收。
审计列表灰底白字已实测复现，根因为选中态主题仅匹配来源列表。`0019` 扩展到
`auditRecords`，导出渲染测试共用同一文本与状态矩阵，先复现审计失败，再验证
来源/审计浅深主题及焦点变化均通过。组件结果不代表 U06 整体验收完成。
后续 `fd211c3` 原生复验已确认：偏好选中项及两个历史版本可读，冲突页中文结果、
严重程度、缺失标题提示和新旧内容正常，浅/深主题选中项可读；设备页未勾选方框可见。
运行文件哈希与安装文件一致，V11 作业 34164563524 成功。未执行同步写操作；
完整错误态、双设备传输和其他交互仍未验收，不将这组检查图作为最终图库。
