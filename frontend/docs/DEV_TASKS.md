...
可选取标签，隐藏并清空独立文本框，不推测路径。来源列表上限 72 像素并可滚动，
正文最低 100 像素，正文与摘要的剩余空间权重为 3:1。新增几何和状态回归通过，
完整管理 12 组与宿主导出检查通过。616c7d7 原生复测已确认旧记录紧凑提示和
新合成目录文件的实际来源链路，采集授权随后恢复全关；有效来源和正文虽可滚动
读到末尾，垂直空间仍拥挤，需继续改善，不作为最终图库验收。
...
来源已有后端链路，原生新元数据、OCR 绑定、权限失败与停用竞态仍待验证；
配置保存成功不能证明采集成功。
