...
- 🟡 一键升级已实现包名/版本/架构校验及安装后的 dpkg、后端、schema、Provider
  健康判定，失败不会误报成功；最终发布门尚缺独立签名、完整 Module E/宿主兼容
  矩阵、可信旧包备份/自动回滚、受控前端重启与最终 V11 图形升级证据；详见
  `docs/DELIVERY_PLAN.md`，不得将现状表述为完整版本管理验收通过。
- ✅ 2026-09-03 修复非交互升级配置冲突：运行配置改由 postinst 从只读模板首装
  创建并在升级时保留；V11 portable 跨 revision 已验证配置摘要不变、后端恢复可用。
