...
通过设置恢复默认 Ctrl+Alt+P 后的全局唤起。旧进程遗留同键记录会使 SDK 更新返回 -2：已按官方接口在确认该键属于 PIXIU 后仅更新动作，避免把自身绑定误判为冲突。真实 SDK 及生产 ShortcutManager 原生探针已验证恢复全局注册；新候选包与完整冲突矩阵仍待验收。现有宿主导出与供应链清单已包含该源码，无新增依赖。
...
2026-09-10 发布 CI 修复：通用管理工作区的快捷键 SDK 夹具补齐 V11 按键查询及列表释放接口，并覆盖已属本应用的相同按键只更新动作。生产 SDK 调用不变，不增加依赖或导出源码；测试夹具仍仅参与现有 CMake 测试目标。完整管理工作区构建及 18 项 CTest 全部通过。
