...
- **提问检索** — 输入模糊问题，展示后端返回的答案 + 置信度 + 延迟，并附**证据卡**（原文详情接口待后端提供，点击当前提示"待提供"）。
- **录入知识** — 粘贴文本或拖入图片（走 OCR），把内容写入长期记忆。
- **自然语言遗忘** — 输入"忘记那张 4 月支出清单"，弹出二次确认对话框后执行精准遗忘。
- **记忆管理面板** — 偏好提取与版本历史、冲突审计记录。
- **多设备同步管理** — PIN/令牌 **配对新设备**加入共享域（QR 令牌页占位）；
  查看节点列表、在线状态与同步进度；一键**解绑**设备。节点上下线与同步事件
  会以系统通知提醒（KYSDK=ON 路径；KYSDK=OFF 降级为系统托盘通知）。
