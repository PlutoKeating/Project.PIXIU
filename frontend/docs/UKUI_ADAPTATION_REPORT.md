...
| A-4 | 窗口装饰与置顶 | 同一宿主装饰；HostWindowPin 经 waylandhelper 读取并切换状态 | f2246f0 自动安装后在 V11/Wayland 验证双向置顶、跨应用遮挡、最小化后快捷键恢复，SDK 与画面一致 | 其余窗口状态、多屏与故障矩阵未完成；Qt 兼容提示不等于原生确认 |
...
| A-7 | .deb 打包 | 根 build/release 的单一完整包 | f2246f0 原生自动作业 34207026274 已成功构建、安装和核对 SDK 生命周期 | 已安装不等于最终全部验收；尚未发布新的最终 Release |
...
此只读程序仅证明窗口识别与读取。随后自动安装完成，从系统菜单启动新进程并
核对文件哈希后，实际点击设置置顶和取消：SDK above 与复选框一致，终端获得
焦点时置顶窗口仍覆盖终端，取消后可被终端覆盖。置顶状态最小化后，实际
Ctrl+Alt+P 恢复同一窗口及进程，置顶保留。最后恢复未置顶；没有发送模型请求
或修改记忆。这些实际操作只覆盖所列路径，不证明全部窗口及平台矩阵。
...
| f2246f0 | 原生作业 34207026274 成功；已安装及菜单新启动进程 SHA-256 同为 ad723e7e0a5f4d095811126c5511d212a94aecb17a940814583c5e14ccd09f3a；真实操作见 3.2 | 该原生包的双向置顶、遮挡与最小化恢复，不等于完整 U11 或最终发布验收 |
