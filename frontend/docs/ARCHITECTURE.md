...
旧 InputBar、MessageList、ThinkingSkeleton、EvidenceCard、专属消息类型与测试/翻译已删除，UiIcons 也已清退；上游宿主消息类型和正式来源阅读器未改动。正式设置页通过 HostWindowPin 控制同一主窗口置顶，不修改启动配置。ON 使用 kysdk-waylandhelper，按进程和窗口标题唯一识别目标，读取实际状态并等待反馈；超时不自动重试切换。OFF 保留 Qt 窗口提示并明确实际效果取决于窗口管理器。组件回归与 V11 真实头文件编译已验证，完整安装后的置顶操作仍待验收；输入保留、离线导航和无障碍按正式会话路径验收。
