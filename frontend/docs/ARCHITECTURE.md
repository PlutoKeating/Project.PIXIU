...
文件采集来源使用同一阅读器内独立只读纯文本区，展示文本/OCR 方法、采集时路径、
UTC 记录时间及“不保证文件仍存在或内容未变化”提示；不提供自动打开文件操作。
路径按 JSON 字符串转义显示，可复制/滚动，区域高度上限 110 像素，不进入正文或
高级 raw。缺失/null 明确标记未记录；非法结构、方法、时间、非绝对路径、非私有域
或不兼容来源类型拒绝展示元数据，正文仍可读。时间展示支持 Unix 秒 0～253402300799。
来源、范围、数据失效切换清空并隐藏旧来源；迟到响应不恢复失效数据。组件与导出
检查通过，原生同版布局及完整主题/语言矩阵仍待验收。复用 Qt Core/Widgets，无新增依赖。
