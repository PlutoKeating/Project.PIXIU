...
宿主对外显示名称为 PIXIU，启动入口不再覆盖主窗口既有 `PIXIU · KylinAgent` 标题。
内部 applicationName、organizationName 与 desktop 文件身份保持不变，用于兼容既有
配置和桌面关联；这不是保留第二套前端。修改仅在导出副本应用，原 submodule 不变。

ADR-0006 已替代独立控制台的产品定位。应用生命周期归唯一宿主管理；当前宿主尚未
创建托盘，关闭最后一个可见主窗口应退出应用，
不再设置无托盘常驻。`HostCloseGuard` 在唯一主窗口处理关闭事件：已有可见对话框
...
或全部记忆统计；当前无历史日期选择器。洞察 HTTP 响应缺失列表时进入错误通道。
